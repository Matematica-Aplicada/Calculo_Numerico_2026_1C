...
\begin{enumerate}[(a)]
\item Escriba el pseudocódigo del método de Horner. Cuente el número de multiplicaciones y sumas y determine la complejidad. 
...
\begin{enumerate}[(a)]
\item Defina $b^{(i)} \in \mathbb{R}^m$ como el vector que coincide con $b$ en todas las coordenadas salvo la $i$-ésima, donde lo reemplazamos con el valor predicho por el modelo ajustado sin la $i$-ésima observación:
\[
b_i^{(i)} = a_i^T \hat{x}^{(-i)}.
\]
