...
En este capítulo estudiamos métodos numéricos para tres clases de ecuaciones diferenciales:
...
En todos los casos la estrategia es la misma: introducir una malla de puntos $\{t_n=t_0+nh\}$ y/o $\{x_j=jh\}$, y reemplazar cada derivada por una \emph{fórmula de diferencias finitas}: una combinación lineal de valores de la función en puntos vecinos de la malla. Aplicando esta sustitución al problema diferencial obtenemos un sistema algebraico cuya solución $\{y_n\}$, $\{u_j\}$ o $\{U_j^n\}$ queremos que aproxime la solución exacta cuando el paso $h\to0$.
...
La fórmula de diferencias forward aproxima $y'(t_n)\approx\frac{y_{n+1}-y_n}{h}$. Evaluando el lado derecho en $t_n$:
\[
  \frac{y_{n+1}-y_n}{h}=\alpha y_n\quad\Longrightarrow\quad y_{n+1}=(1+h\alpha)\,y_n.
\]
Llamamos $z:=h\alpha$ y $R_E:=1+z$. Por inducción, $y_n=R_E^n\,y_0=(1+h\alpha)^n y_0$.

Fijemos $t=t_n=nh$. Cuando $h\to0$ con $t$ fijo ($n=t/h\to\infty$):
\[
  y_n=(1+h\alpha)^{t/h}\,y_0\;\xrightarrow{\;h\to0\;}\;e^{\alpha t}\,y_0=y(t).
\]

\paragraph{Comportamiento cualitativo para $h$ fijo.}
La solución numérica es $y_n=(1+h\alpha)^n y_0$, y crece o decae según $|1+h\alpha|$ sea mayor o menor que~1. Para $\alpha<0$:
\begin{itemize}
  \item Si $h<2/|\alpha|$: $|1+h\alpha|<1$ y $|y_n|\to0$. El método reproduce el decaimiento. 
  \item Si $h=2/|\alpha|$: $1+h\alpha=-1$, y $y_n=(-1)^n y_0$ oscila indefinidamente. 
  \item Si $h>2/|\alpha|$: $|1+h\alpha|>1$ y $|y_n|\to\infty$, aunque la solución exacta decae. 
\end{itemize}
Por ejemplo, con $\alpha=-10$ y $h=0.3>2/10$: $1+h\alpha=-2$, y $y_n=(-2)^n y_0$ diverge en módulo. La convergencia asintótica ($h\to0$) no garantiza un comportamiento correcto para pasos finitos.

\subsubsection*{Euler implícito}

La fórmula de diferencias backward evalúa el lado derecho en $t_{n+1}$:
\[
  \frac{y_{n+1}-y_n}{h}=\alpha y_{n+1}\quad\Longrightarrow\quad y_{n+1}=\frac{1}{1-h\alpha}\,y_n=:R_I\,y_n.
\]
Por inducción, $y_n=R_I^n\,y_0=\left(\dfrac{1}{1-h\alpha}\right)^n y_0$.

\paragraph{Convergencia cuando $h\to0$.}
Con $t=nh$, análogamente al caso explícito, $\log(1/(1-h\alpha))=h\alpha+O(h^2)$, luego
\[
  y_n=\left(\frac{1}{1-h\alpha}\right)^{t/h} y_0\;\xrightarrow{\;h\to0\;}\;e^{\alpha t}\,y_0=y(t).
\]

\paragraph{Comportamiento cualitativo para $h$ fijo.}
Para $\alpha<0$ y \emph{cualquier} $h>0$: $1-h\alpha>1>0$, luego
\[
  |R_I|=\frac{1}{1-h\alpha}=\frac{1}{1+h|\alpha|}<1.
\]
Entonces $|y_n|=|R_I|^n|y_0|\to0$ para todo $h>0$: Euler implícito reproduce el decaimiento de la solución exacta sin ninguna restricción sobre el paso.

\bigskip
\begin{remark}
Ambos métodos convergen cuando $h\to0$, pero difieren cualitativamente para $h$ fijo: Euler explícito puede producir soluciones divergentes si el paso supera el umbral $2/|\alpha|$, mientras que Euler implícito siempre decae. Este contraste se vuelve algorítmicamente crítico cuando $|\alpha|\gg1$ (problemas rígidos), obligando al método explícito a usar pasos extremadamente pequeños. Este fenómeno se estudia en la Sección~\ref{sec:stiff_new}.
\end{remark}

\subsection{Métodos de un paso}\label{sec:metodos_pvi_new}

Seguimos con el PVI $y'=f(t,y)$, $y(t_0)=y_0$ sobre la malla $\{t_n=t_0+nh\}_{n=0}^N$ con $Nh=T-t_0$.

\subsubsection*{Taylor de orden 2 y la familia RK2}

El método de Euler explícito tiene orden~1: el error global es $O(h)$. Para obtener orden~2 expandimos la solución exacta dos pasos de Taylor:
\[
  y(t_{n+1})=y(t_n)+h\,y'(t_n)+\frac{h^2}{2}\,y''(t_n)+O(h^3).
\]
Como $y'=f$ y $y''=f_t+f_y\,f$ (regla de la cadena), el \emph{método de Taylor de orden~2} es
\[
  y_{n+1}=y_n+h\,f(t_n,y_n)+\frac{h^2}{2}\bigl(f_t(t_n,y_n)+f_y(t_n,y_n)\,f(t_n,y_n)\bigr).
\]
...
Para evitar diferenciar $f$, se aproxima el término de orden $h^2$ evaluando $f$ en un punto auxiliar. Se propone el esquema
\begin{align*}
  k_1&=f(t_n,\,y_n),\\
...
\[
  k_2 = f + c_2 h\,f_t + a_{21} h k_1 f_y + O(h^2) = f + h(c_2 f_t + a_{21} f\,f_y)+O(h^2).
\]
Entonces $y_{n+1}=y_n+h(b_1+b_2)f+h^2 b_2(c_2 f_t+a_{21}f\,f_y)+O(h^3)$. Comparando con Taylor:
\[
...
\begin{itemize}
  ...
  Equivalentemente: $k_1=f(t_n,y_n)$, $k_2=f(t_n+h,\,y_n+hk_1)$, $y_{n+1}=y_n+\tfrac{h}{2}(k_1+k_2)$.
  \item \textbf{Punto medio explícito}: $b_1=0$, $b_2=1$, $c_2=a_{21}=\tfrac{1}{2}$.
  Equivalentemente: $k_1=f(t_n,y_n)$, $y_{n+1}=y_n+h\,f(t_n+\tfrac{h}{2},\,y_n+\tfrac{h}{2}k_1)$.
\end{itemize}
...
\end{equation}
donde $\Phi:[t_0,T]\times\mathbb{R}\times[0,h_{\max}]\to\mathbb{R}$ es la \emph{función de incremento} que caracteriza al método. La tabla siguiente resume los métodos vistos hasta aquí.

\bigskip
\begin{center}
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Método} & \textbf{Función de incremento $\Phi(t,y,h)$} & \textbf{Orden} & \textbf{Tipo} \\
\midrule
Euler explícito & $f(t,y)$ & 1 & expl.\ \\[4pt]
Euler implícito & $\tilde{y}$ tal que $\tilde{y}=f\!\left(t+h,\,y+h\tilde{y}\right)$ & 1 & impl.\ \\[4pt]
Taylor orden~2  & $f(t,y)+\dfrac{h}{2}\!\left(f_t(t,y)+f_y(t,y)\,f(t,y)\right)$ & 2 & expl.\ \\[8pt]
Heun (RK2)      & $\dfrac{1}{2}\!\left[f(t,y)+f\!\left(t+h,\,y+hf(t,y)\right)\right]$ & 2 & expl.\ \\[6pt]
Punto medio     & $f\!\left(t+\tfrac{h}{2},\,y+\tfrac{h}{2}f(t,y)\right)$ & 2 & expl.\ \\[4pt]
\bottomrule
\end{tabular}
\end{center}
\bigskip
...
\[
  \tilde y = f(t_{n+1}, y_n+h\tilde y).
\]
En problemas lineales, por ejemplo si $f(t,y)=a(t)y+b(t)$, la ecuación se resuelve explícitamente:
\[
  \tilde y = a(t_{n+1})(y_n+h\tilde y)+b(t_{n+1})
  \quad\Longrightarrow\quad
  \tilde y = \frac{a(t_{n+1})y_n+b(t_{n+1})}{1-ha(t_{n+1})},
\]
siempre que $1-ha(t_{n+1})\neq0$.
\end{remark}
...
\[
  \max_{\substack{0\le n\le N\\t_n\le T}}|y_n-y(t_n)|\;\xrightarrow{\;h\to0\;}\;0
\]
...
\[
  \tau_{n+1}(h)\;:=\;\frac{y(t_{n+1})-y(t_n)}{h}-\Phi\!\bigl(t_n,\,y(t_n),\,h\bigr).
\]
El método es \textbf{consistente} si $\|\tau(h)\|_\infty:=\max_n|\tau_{n+1}(h)|\to0$ cuando $h\to0$. Si $\|\tau(h)\|_\infty=O(h^p)$, decimos que tiene \textbf{orden~$p$}.

Equivalentemente, el método es consistente si y solo si $\Phi(t,y,0)=f(t,y)$ para todo $(t,y)$.
\end{definition}

La condición $\Phi(t,y,0)=f(t,y)$ es inmediata para Euler explícito ($\Phi=f$). Para RK2 se verifica sustituyendo $h=0$: $k_2\to f(t,y)=k_1$ y $\Phi\to b_1 k_1+b_2 k_2=(b_1+b_2)f=f$.

\begin{definition}[Estabilidad de un método de un paso]\label{def:estab_datos_new}
El método es \textbf{estable} en $[t_0,T]$ si existen $h_0>0$ y $C_T>0$ independientes de $h$ tales que, para todo $0<h\le h_0$, toda pareja de datos $y_0,\tilde y_0$ y todo $n$ con $t_n\le T$,
\[
...
Si $\Phi$ es Lipschitz en $y$ con constante $K$ (uniformemente en $t$ y $h$), entonces el método es estable con $C_T = e^{KT}$. En efecto, sea $\delta_n := y_n - \tilde y_n$ la diferencia de dos soluciones numéricas con datos distintos. Ambas satisfacen el mismo esquema, luego
\[
  \delta_{n+1} = \delta_n + h\bigl[\Phi(t_n,y_n,h) - \Phi(t_n,\tilde y_n,h)\bigr],
\]
y la condición de Lipschitz da $|\delta_{n+1}| \le (1+hK)|\delta_n|$. Iterando y aplicando el Lema~\ref{lem:exp_bound_new}: $|\delta_n| \le (1+hK)^n|\delta_0| \le e^{hKn}|\delta_0| \le e^{KT}|y_0-\tilde y_0|$.

Para los métodos de la tabla: Euler explícito tiene $\Phi=f$, por lo que $K=L$ (la constante de Lipschitz de $f$). Euler implícito tiene $K_I(h)=L/(1+hL)<L$. Para los métodos RK2, $\Phi$ hereda el Lipschitz de $f$ con una constante explícitamente computable. En todos los casos, si $f$ es Lipschitz entonces $\Phi$ también lo es, y la estabilidad queda garantizada \emph{sin verificación adicional}.
\end{remark}
...
Sea $y'=f(t,y)$ con $f$ Lipschitz en $y$ con constante $L$ en $[t_0,T]\times\mathbb{R}$. Supongamos que $\Phi$ es Lipschitz en $y$ con constante $K$ uniformemente en $t$ y $h$. Entonces:
\begin{enumerate}
  \item \emph{(Consistencia $\Rightarrow$ convergencia, con orden.)} Si $|\tau_n(h)|\le Ch^p$ para todo $n$, entonces
  \[
    \max_{0\le n\le N}|y(t_n)-y_n|\le \frac{e^{K(T-t_0)}-1}{K}\,Ch^p.
  \]
...
\begin{proof}
\textit{(1)} Sea $e_n:=y(t_n)-y_n$. Entonces $e_0=0$ y
\[
  e_{n+1}=y(t_{n+1})-y_{n+1}
  =\underbrace{\bigl[y(t_{n+1})-y(t_n)-h\,\Phi(t_n,y(t_n),h)\bigr]}_{h\,\tau_{n+1}}
  +h\bigl[\Phi(t_n,y(t_n),h)-\Phi(t_n,y_n,h)\bigr].
\]
Por la condición de Lipschitz sobre $\Phi$: $|e_{n+1}|\le (1+hK)|e_n|+h|\tau_{n+1}|$. Como $e_0=0$, iterando y usando el Lema~\ref{lem:exp_bound_new} con $x=hK$:
\[
  |e_n|\le h\sum_{j=1}^{n}e^{hK(n-j)}|\tau_j|\le nh\cdot e^{hKn}\cdot Ch^p\le T\,e^{KT}\cdot Ch^p.
\]

\textit{(2)} La consistencia es la condición $\Phi(t,y,0)=f(t,y)$. Si el método converge para la ecuación $y'=f$, en particular la solución numérica $y_1=y_0+h\Phi(t_0,y_0,h)$ satisface $y_1\to y(t_0+h)=y_0+hf(t_0,y_0)+O(h^2)$ cuando $h\to0$. Dividiendo por $h$ y tomando $h\to0$: $\Phi(t_0,y_0,0)=f(t_0,y_0)$, que es la consistencia.
\end{proof}

\begin{remark}
La dirección (1) es la más importante para la práctica: basta verificar que el error local es $O(h^{p+1})$ (es decir, que el método tiene orden~$p$) para concluir que el error global es $O(h^p)$. La constante de la cota es $C' = \frac{e^{K(T-t_0)}-1}{K}\,C$, donde $K$ es la constante de Lipschitz de $\Phi$ cuya existencia garantiza la estabilidad (Definición~\ref{def:estab_datos_new} y el remark anterior).
\end{remark}

\subsection{Problemas rígidos o ``stiff''}\label{sec:stiff_new}

En la Sección~\ref{sec:ecuacion_test_new} vimos que Euler explícito e implícito difieren cualitativamente en la ecuación $y'=\alpha y$ para $h$ fijo: el método explícito diverge si $h>2/|\alpha|$, mientras que el implícito decae para cualquier $h>0$. Aquí usamos el Teorema~\ref{thm:one_step_lax_new} para entender de manera más precisa qué paso impone cada método.
...
$\Phi_E(t,y,h)=\lambda y$, que es lineal en $y$:
\[
  |\Phi_E(t,y_1,h)-\Phi_E(t,y_2,h)|=|\lambda|\,|y_1-y_2|.
\]
La constante de Lipschitz es $K_E=|\lambda|$. El Teorema~\ref{thm:one_step_lax_new} da
\[
  \max_n|e_n|\;\le\;T\,e^{|\lambda|T}\,Ch,
\]
donde $C=|\lambda|^2/2$ viene del error de truncamiento $|\tau_n|=\frac{h\lambda^2}{2}|y(t_n)|\le\frac{h\lambda^2}{2}|y_0|$. La cota crece \emph{exponencialmente} con $|\lambda|$. Para garantizar error $<\epsilon$ con esta estimación se necesitaría
\[
  h\;<\;\frac{2\epsilon}{\lambda^2 T\,e^{|\lambda|T}},
\]
un paso astronómicamente pequeño para $|\lambda|\gg1$.

\paragraph{Euler implícito.}
El esquema resuelve $y_{n+1}=y_n+h\lambda y_{n+1}$, dando $y_{n+1}=y_n/(1-h\lambda)$. La función de incremento es
\[
  \Phi_I(t,y,h)=\frac{\lambda y}{1-h\lambda},
\]
también lineal en $y$, con constante de Lipschitz
\[
  K_I(h)=\frac{|\lambda|}{1-h\lambda}=\frac{|\lambda|}{1+h|\lambda|}.
\]
Nótese que $K_I(h)<|\lambda|=K_E$ para todo $h>0$. Más importante: $K_I$ es acotada superiormente por $1/h$, independientemente de $|\lambda|$:
\[
  K_I(h)=\frac{|\lambda|}{1+h|\lambda|}\;\le\;\frac{1}{h},
\]
con $K_I(h)\to 1/h$ cuando $|\lambda|\to\infty$ con $h$ fijo. Aplicando el Teorema~\ref{thm:one_step_lax_new} con esta cota:
\[
  \max_n|e_n|\;\le\;T\,e^{K_I(h)\,T}\,Ch\;\le\;T\,e^{T/h}\,Ch.
\]
Esta cota es independiente de $|\lambda|$: para cualquier valor de la rigidez, el error de Euler implícito queda controlado sin restringir $h$ en función de $|\lambda|$.

\begin{remark}
El contraste entre los dos métodos se resume así: la cota del Teorema~\ref{thm:one_step_lax_new} para Euler explícito tiene el factor $e^{|\lambda|T}$ que crece sin límite con la rigidez; la cota para Euler implícito tiene el factor $e^{T/h}$ que solo depende del paso y del intervalo, no de $|\lambda|$. En la práctica, la restricción de paso de Euler explícito ($h\lesssim 2/|\lambda|$) no proviene de la exactitud sino de la \emph{estabilidad}: para $h>2/|\lambda|$ la solución numérica diverge (como vimos en la Sección~\ref{sec:ecuacion_test_new}), mientras que Euler implícito es estable para todo $h>0$.
