...
Para calcular $\|A^{-1}\|_2$, usemos directamente la SVD. Si
\[
A=U\Sigma V^*,\qquad \Sigma=\operatorname{diag}(\sigma_1,\ldots,\sigma_n),\ \sigma_n>0,
\]
entonces
\[
A^{-1}=(U\Sigma V^*)^{-1}=V\Sigma^{-1}U^*=V\operatorname{diag}(1/\sigma_1,\ldots,1/\sigma_n)U^*.
\]
Así, los valores singulares de $A^{-1}$ son $1/\sigma_1,\ldots,1/\sigma_n$, y ordenados quedan
$1/\sigma_n\ge\cdots\ge 1/\sigma_1$. Aplicando el teorema anterior,
$\|A^{-1}\|_2=1/\sigma_n$. Por lo tanto:
\[
...
El problema admite una lectura lineal inmediata.  Definimos el \textbf{operador de evaluación}
\[
\Phi\colon\mathcal{P}_n \to \mathbb{F}^{n+1},\qquad \Phi(p) = \bigl(p(x_0),\ldots,p(x_n)\bigr).
\]
Interpolar equivale a invertir $\Phi$.  Como $\dim\mathcal{P}_n = \dim\mathbb{F}^{n+1} = n+1$, basta probar que $\Phi$ es inyectiva.

\begin{theorem}[Existencia y unicidad]\label{thm:existencia_unicidad}
Si los nodos $x_0,\ldots,x_n$ son distintos, $\Phi$ es un isomorfismo.  En particular, el problema de interpolación tiene solución única.
\end{theorem}
\begin{proof}
Sea $p\in\ker\Phi$, es decir $p(x_i)=0$ para $i=0,\ldots,n$.  Un polinomio no nulo de grado $\leq n$ tiene a lo sumo $n$ raíces (Teorema~\ref{thm:raices_polinomio}, Apéndice~A), pero $p$ tiene $n+1$ raíces; luego $p\equiv 0$ y $\Phi$ es inyectiva.
\end{proof}
...
\begin{remark}[Mal condicionamiento de Vandermonde]\label{rmk:vandermonde_cond}
Resolver el sistema $Va=y$ es, en general, un método numéricamente \emph{inadecuado} para calcular el interpolante: $\kappa_2(V)$ crece exponencialmente con~$n$.

Para ver esto, usamos la caracterización variacional $\sigma_{\min}(V) = \min_{a\neq 0}\|Va\|_2/\|a\|_2$.  Si los nodos $x_0,\ldots,x_n$ están en $[-1,1]$, la suma $\|Va\|_2^2 = \sum_{i=0}^n p(x_i)^2$ (donde $p$ tiene coeficientes~$a$) se comporta como una suma de Riemann de $\int_{-1}^1 p(x)^2\,dx$ escalada por un factor $O(n)$.  Para el polinomio de prueba $p(x)=(1-x^2)^n$, la integral $\int_{-1}^1 (1-x^2)^{2n}\,dx\to 0$, mientras que, usando la identidad $\sum_{k=0}^n\binom{n}{k}^2 = \binom{2n}{n}$, se verifica que $\|a\|_2^2 \geq 4^n/(2n+1)$.  Esto fuerza que $\sigma_{\min}(V)$ decaiga exponencialmente y, en consecuencia, $\kappa_2(V)$ crezca exponencialmente.
\end{remark}
