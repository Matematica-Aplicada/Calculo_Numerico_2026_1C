...
\fi
\chapter{Ecuaciones No Lineales}

Encontrar las raíces de una función $f:\mathbb{R}\to\mathbb{R}$,
es decir resolver $f(x)=0$, es uno de los problemas más recurrentes en la práctica
científica y en la ingeniería.
En este capítulo desarrollamos cuatro métodos clásicos:
bisección, iteración de punto fijo, Newton y la secante.
El hilo conductor es la construcción de sucesiones $\{x_k\}$
convergentes a una raíz, y el análisis cuantitativo de su velocidad de convergencia.

\section{Motivación: ajuste de curvas y ecuaciones no lineales}
\label{sec:motivacion_nolineal}

Partimos de un problema unificador: tenemos datos $(t_i,y_i)$ y un modelo
$m(t;\theta)$ con parámetros $\theta\in\mathbb{R}^p$.
Queremos hallar $\theta$ de modo que el modelo reproduzca los datos, minimizando
la pérdida cuadrática
\[
\Phi(\theta)=\frac{1}{2}\sum_{i=1}^N\bigl(m(t_i;\theta)-y_i\bigr)^2.
\]
Los puntos críticos satisfacen $\nabla\Phi(\theta)=0$, que en general es un
\emph{sistema no lineal}.  Si $p=1$ se reduce a una sola ecuación no lineal en una variable.

\begin{example}[Oscilador amortiguado]
En mecánica, un modelo clásico es
$x(t;A,\beta,\omega,\varphi)=A\,e^{-\beta t}\cos(\omega t+\varphi)$.
El ajuste de $(A,\beta,\omega,\varphi)$ por mínimos cuadrados contra datos
$(t_i,x_i^{\mathrm{obs}})$ es un problema no lineal: los parámetros aparecen en
exponentes y argumentos de coseno, y no existe un cambio de variables que lo
linealice en todos los parámetros simultáneamente.
\end{example}

\begin{example}[Ley de desplazamiento de Wien]\label{ex:wien}
La ley de Planck para la radiancia espectral es
\[
...
Este ejemplo es ubícuo: conecta la temperatura de las estrellas con su color,
y es la base de la pirometría óptica y de las cámaras térmicas.
\end{example}
