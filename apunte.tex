...
Este capítulo está organizado en tres clases:
\begin{enumerate}
\item \textbf{Clase 1:} métodos básicos en $\mathbb{R}$ (punto fijo, bisección, Newton en una variable).
\item \textbf{Clase 2:} optimización y descenso por gradiente.
\item \textbf{Clase 3:} método de Newton en $\mathbb{R}^n$ para sistemas y optimización.
\end{enumerate}
El hilo conductor es siempre el mismo: construir una iteración
\[
x^{(k+1)} = G(x^{(k)})
\]
y entender cuándo converge, con qué velocidad y a qué solución converge.


\section{Clase 1: Métodos básicos en $\mathbb{R}$}

\subsection{Motivación: ajuste de parámetros de modelos contra datos}

Partimos de un problema unificador: tenemos datos $(t_i,y_i)$ y un modelo $m(t;\theta)$ con parámetros $\theta\in\mathbb R^p$. Queremos ajustar $\theta$ para que el modelo reproduzca los datos.

Usaremos la pérdida cuadrática
\[
\Phi(\theta)=\frac{1}{2}\sum_{i=1}^N\big(m(t_i;\theta)-y_i\big)^2.
\]
Los puntos críticos satisfacen
\[
\nabla\Phi(\theta)=0,
\]
que en general es un sistema no lineal.  Si $p=1$, queda una ecuación no lineal en una variable.
...
\]
Si disponemos de datos $(t_i,x_i^{\mathrm{obs}})$, el ajuste por mínimos cuadrados se plantea como
\[
\Phi(A,\beta,\omega,\varphi)=\frac{1}{2}\sum_{i=1}^N\Big(Ae^{-\beta t_i}\cos(\omega t_i+\varphi)-x_i^{\mathrm{obs}}\Big)^2.
\]
El ajuste de $(A,\beta,\omega,\varphi)$ por mínimos cuadrados es no lineal: los parámetros aparecen en productos, exponentes y funciones trigonométricas, y no existe un cambio simple que lo reduzca a un problema lineal en todos los parámetros simultáneamente.
\end{example}

\begin{example}[Termodinámica: ley de desplazamiento de Wien]
La ley de Planck para la radiancia espectral en función de la longitud de onda es
\[
I(\lambda;T)=\frac{2hc^2}{\lambda^5}\,\frac{1}{e^{hc/(\lambda k_B T)}-1}.
\]
Si queremos ubicar, para una temperatura fija $T$, la longitud de onda de máxima emisión $\lambda_{\max}$, derivamos respecto de $\lambda$ e imponemos
\[
\frac{\partial I}{\partial \lambda}(\lambda;T)=0.
\]
Introduciendo la variable adimensional
\[
x=\frac{hc}{\lambda k_B T},
\]
la ecuación de primer orden se reduce a la trascendente
\[
5-x=5e^{-x}.
\]
Es una ecuación no lineal en una dimensión para $x\in\mathbb R$, sin forma cerrada elemental, y se resuelve numéricamente. Su solución relevante es
\[
x\approx 4.965114.
\]
Por lo tanto,
\[
\lambda_{\max}T=\frac{hc}{xk_B}\approx 2.897\times 10^{-3}\;\mathrm{m\cdot K}.
\]
Esta constante permite estimar la temperatura de estrellas a partir de su color (pico de emisión), y es la base de instrumentos como pirómetros ópticos y cámaras térmicas.
\end{example}

\begin{example}[Caída con arrastre cuadrático]
Para caída vertical con fuerza de arrastre proporcional a $v^2$, una forma del modelo para la velocidad es
\[
v(t;v_\infty)=v_\infty\tanh\!\left(\frac{gt}{v_\infty}\right).
\]
Si medimos pares $(t_i,v_i^{\mathrm{obs}})$, minimizamos
\[
\Phi(v_\infty)=\frac{1}{2}\sum_{i=1}^N\big(v(t_i;v_\infty)-v_i^{\mathrm{obs}}\big)^2.
\]
Ajustar $v_\infty$ contra datos experimentales también produce una ecuación no lineal.
\end{example}

\begin{example}[Modelo sigmoideo para probabilidad]
Si queremos modelar una probabilidad en función de una variable $z$, usamos
\[
p(z;w,b)=\sigma(wz+b),\qquad \sigma(s)=\frac{1}{1+e^{-s}}.
\]
Con datos $(z_i,y_i)$, $y_i\in\{0,1\}$, podemos ajustar $(w,b)$ con pérdida cuadrática:
\[
\Phi(w,b)=\frac{1}{2}\sum_{i=1}^N\big(\sigma(wz_i+b)-y_i\big)^2.
\]
Las ecuaciones de primer orden son no lineales por la presencia de la sigmoide.
\end{example}

\begin{remark}[Red neuronal de una neurona y generalizaciones]
El modelo sigmoideo anterior es exactamente una \textbf{red neuronal de una neurona}.

En dimensión $d$, el escalar $wz+b$ se reemplaza por un producto interno:
\[
p(x;w,b)=\sigma(w^Tx+b),\qquad w\in\mathbb R^d.
\]
Esto sigue siendo una neurona, ahora con entrada vectorial.

Una red de varias neuronas en una capa escribe
\[
F(x)=\sum_{j=1}^m \alpha_j\,\sigma(w_j^Tx+b_j),
\]
y una red multicapa se obtiene por \textbf{composición de funciones}:
\[
F(x)=F_L\circ F_{L-1}\circ\cdots\circ F_1(x).
\]
\end{remark}

\begin{remark}[Elección de la pérdida y regularización]
En muchas aplicaciones no se usa pérdida cuadrática, sino otras métricas de error (por ejemplo entropía cruzada o pérdidas de tipo Huber), y con frecuencia se añaden términos de regularización como Tychonoff ($\ell_2$) o norma $\ell_1$.

Estas elecciones suelen surgir de consideraciones probabilísticas/estadísticas y de robustez.  Históricamente, la pérdida cuadrática se usa mucho porque es la opción más simple analíticamente y conduce a derivaciones transparentes.
\end{remark}

\subsection{Iteraciones de punto fijo}

\begin{definition}[Método de punto fijo]\label{def:punto_fijo}
Dada una función $g:I\to I$, la iteración de punto fijo es
\[
...
\begin{theorem}[Contracción en $\mathbb{R}$]
Sea $I=[a,b]$ y $g:I\to I$ derivable con
\[
\sup_{x\in I}|g'(x)|\le q<1.
\]
Entonces existe un único punto fijo $x^*\in I$ y, para todo $x_0\in I$, la sucesión converge linealmente:
\[
|x_{k}-x^*|\le q^k|x_0-x^*|.
\]
\end{theorem}

\begin{example}[Punto fijo del coseno]
La ecuación $x=\cos x$ equivale a un problema de punto fijo con $g(x)=\cos x$.  En $I=[0,1]$ vale $g(I)\subset I$ y
\[
|g'(x)|=|\sin x|\le \sin(1)<1,
\]
de modo que la iteración $x_{k+1}=\cos(x_k)$ converge al único punto fijo.
\end{example}

\section{El método de la bisección}\label{sec:biseccion}

El método de bisección usa solo continuidad y cambio de signo.

\begin{definition}[Método de la bisección]\label{def:biseccion}
Dados $a_0<b_0$ con $f(a_0)f(b_0)<0$, definimos
\[
m_k=\frac{a_k+b_k}{2}
\]
y elegimos el subintervalo donde hay cambio de signo.
\end{definition}

\begin{theorem}[Convergencia de bisección]\label{thm:convergencia_biseccion}
Si $f\in C([a,b])$ y $f(a)f(b)<0$, entonces
\[
|m_k-x^*|\le \frac{b-a}{2^{k+1}},
\]
para alguna raíz $x^*\in[a,b]$.
\end{theorem}

La convergencia es lineal (factor $1/2$), robusta y lenta.

\section{El método de Newton en una variable}\label{sec:newton_1d}

\begin{definition}[Newton en $\mathbb{R}$]\label{def:newton_1d}
Para resolver $f(x)=0$:
\[
...
\begin{proposition}[Newton como punto fijo]
Definiendo
\[
g(x)=x-\frac{f(x)}{f'(x)},
\]
la iteración de Newton es $x_{k+1}=g(x_k)$.  Si $x^*$ es raíz simple de $f$, entonces
\[
g'(x^*)=0,
\]
lo que explica su orden cuadrático.
\end{proposition}

\begin{theorem}[Convergencia cuadrática de Newton en 1D]\label{thm:newton_1d_conv}
Si $f\in C^2$ y $x^*$ es raíz simple, existe $\delta>0$ tal que, si $|x_0-x^*|<\delta$,
\[
...
\section{Clase 2: Optimización y descenso por gradiente}
\label{sec:descenso_gradiente}

\subsection{Dirección de máximo crecimiento y método}

Para $\varphi:\mathbb{R}^n\to\mathbb{R}$ diferenciable, el gradiente apunta en la dirección de crecimiento máximo.  Por eso descendemos en $-\nabla\varphi(x)$.

\begin{definition}[Descenso por gradiente]\label{def:descenso_gradiente}
Con paso constante $\alpha>0$:
\[
x_{k+1}=x_k-\alpha\nabla\varphi(x_k).
\]
\end{definition}

\begin{definition}[Suavidad y convexidad fuerte]
Diremos que $\varphi$ es $L$-suave si $\nabla\varphi$ es $L$-Lipschitz, y $\mu$-fuertemente convexa si
\[
\varphi(y)\ge \varphi(x)+\nabla\varphi(x)^T(y-x)+\frac{\mu}{2}\|y-x\|^2.
\]
\end{definition}

\begin{theorem}[Convergencia con $L$-suavidad]
Sea $\varphi$ convexa y $L$-suave. Con $\alpha=1/L$ se tiene, para todo $k\ge1$,
\[
\varphi(x_k)-\varphi(x^*)\le \frac{L\|x_0-x^*\|^2}{2k}.
\]
En particular, el orden es $O(1/k)$ y puede ser lento para alta precisión.
\end{theorem}

\begin{theorem}[Refinamiento fuertemente convexo]\label{thm:gd_convergence}
Si además $\varphi$ es $\mu$-fuertemente convexa, con $\kappa=L/\mu$,
\[
\|x_k-x^*\|^2\le \left(1-\frac{1}{\kappa}\right)^k\|x_0-x^*\|^2.
\]
La convergencia pasa a ser lineal, pero para $\kappa\gg1$ sigue siendo lenta.
\end{theorem}

\subsection{Aplicaciones: ajuste de curvas, logística y modelo deep linear}

\textbf{Curve fitting.} En cuadrados mínimos, minimizamos
\[
\frac{1}{2}\sum_{i=1}^N\big(F(x^{(i)};\theta)-y^{(i)}\big)^2.
\]

\textbf{Regresión logística (una neurona).} Para etiquetas $y_i\in\{0,1\}$ y variables $z_i$, modelamos
\[
p_i=\sigma(wz_i+b),\qquad \sigma(t)=\frac{1}{1+e^{-t}},
\]
y ajustamos parámetros con pérdida cuadrática
\[
\mathcal L(w,b)=\frac{1}{2}\sum_{i=1}^N\big(p_i-y_i\big)^2.
\]
Las ecuaciones de primer orden en $(w,b)$ son no lineales por la composición con la sigmoide.

\textbf{Deep linear.} En un modelo lineal profundo,
\[
\hat y = W_LW_{L-1}\cdots W_1x,
\]
y el problema de entrenamiento es
\[
\min_{W_1,\dots,W_L}\;\frac{1}{2N}\sum_{i=1}^N\|W_L\cdots W_1x^{(i)}-y^{(i)}\|^2.
\]
Aunque cada capa sea lineal, la dependencia conjunta en los parámetros es no lineal.

\subsection{Cuadráticas: GD y sistemas lineales iterativos}

Consideremos
\[
\varphi(x)=\frac{1}{2}\|Ax-b\|_2^2,\qquad \nabla\varphi(x)=A^T(Ax-b).
\]
El descenso por gradiente con paso $\eta$ es
\[
x_{k+1}=x_k-\eta A^T(Ax_k-b).
\]
Esto es un método iterativo lineal para las ecuaciones normales $A^TAx=A^Tb$:
\[
x_{k+1}=(I-\eta A^TA)x_k+\eta A^Tb.
\]
Es el método de Richardson/Landweber. Converge para
\[
0<\eta<\frac{2}{\|A\|_2^2}.
\]

\begin{theorem}[Límite de norma mínima por SVD]
Sea $A=U\Sigma V^T$ y $0<\eta<2/\|A\|_2^2$.  Si $x_0\in\operatorname{Im}(A^T)$ (en particular $x_0=0$), entonces la iteración anterior converge a
\[
x_\infty=A^+b,
\]
que es el minimizador de norma mínima del problema de cuadrados mínimos.
\end{theorem}

\begin{proof}
En base SVD, cada componente sobre $v_i$ con $\sigma_i>0$ verifica
\[
c_i^{k+1}=(1-\eta\sigma_i^2)c_i^k+\eta\sigma_i\,u_i^Tb,
\]
y por $|1-\eta\sigma_i^2|<1$ converge a $(u_i^Tb)/\sigma_i$.  En $\ker(A)$, la componente permanece constante; si $x_0\in\operatorname{Im}(A^T)$, esa componente es cero para todo $k$.  Luego
\[
x_\infty=\sum_{\sigma_i>0}\frac{u_i^Tb}{\sigma_i}v_i=A^+b.
\]
\end{proof}


\section{Clase 3: Newton en $\mathbb{R}^n$ para sistemas y optimización}
\label{sec:newton_nd}

\subsection{Sistemas no lineales}

Para $F:\mathbb{R}^n\to\mathbb{R}^n$, la iteración de Newton es
\[
J_F(x_k)\,\delta_k=-F(x_k),\qquad x_{k+1}=x_k+\delta_k.
\]

\begin{theorem}[Convergencia cuadrática de Newton en $\mathbb{R}^n$]\label{thm:newton_nd_conv}
Si $F(x^*)=0$, $J_F(x^*)$ es invertible y $J_F$ es Lipschitz en un entorno de $x^*$, entonces para $x_0$ suficientemente cercano a $x^*$:
\[
\|x_{k+1}-x^*\|\le C\|x_k-x^*\|^2.
\]
\end{theorem}

\subsection{Optimización por Newton}

Para minimizar $\varphi$, aplicamos Newton a $F=\nabla\varphi$:
\[
x_{k+1}=x_k-[\nabla^2\varphi(x_k)]^{-1}\nabla\varphi(x_k).
\]
Si $\nabla^2\varphi(x^*)\succ0$ y estamos en vecindad del minimizador, la convergencia es cuadrática.

\begin{remark}[Comparación final]
\begin{itemize}
\item Gradiente: bajo costo por iteración y mejor comportamiento global.
\item Newton: mayor costo por iteración, pero convergencia local cuadrática.
\end{itemize}
La práctica moderna combina ambos (line search, damping, cuasi-Newton, métodos de confianza).
\end{remark}
