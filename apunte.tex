...
\end{theorem}
\begin{proof}
Aplicamos el Teorema~\ref{thm:error_trapecio} en cada subintervalo $[x_j,x_{j+1}]$ (todos de longitud $h$).  Para cada $j=0,\dots,M-1$ existe $\eta_j\in(x_j,x_{j+1})$ tal que
\[
E_j:=\int_{x_j}^{x_{j+1}}f(x)\,dx-T(f;x_j,x_{j+1})=-\frac{h^3}{12}\,f''(\eta_j).
\]
Sumando los errores locales:
\[
E:=\int_a^b f(x)\,dx-T_M(f)=\sum_{j=0}^{M-1}E_j
=-\frac{h^3}{12}\sum_{j=0}^{M-1}f''(\eta_j).
\]

\textbf{Paso clave ("se come un orden"):}
cada término local es de tamaño $O(h^3)$, pero hay $M=(b-a)/h$ términos. Por lo tanto, en magnitud total
\[
|E|\le \frac{h^3}{12}\sum_{j=0}^{M-1}|f''(\eta_j)|
\le \frac{h^3}{12}\,M\,\|f''\|_{\infty,[a,b]}
=\frac{b-a}{12}\,h^2\,\|f''\|_{\infty,[a,b]}.
\]
Aquí se ve explícitamente la pérdida de un orden: \(M\sim h^{-1}\), entonces
\[
M\cdot h^3 \sim h^{-1}\cdot h^3 = h^2.
\]

Para obtener la forma exacta con un único punto, escribimos
\[
\sum_{j=0}^{M-1}f''(\eta_j)=M\,\overline s,
\qquad
\overline s:=\frac1M\sum_{j=0}^{M-1}f''(\eta_j).
\]
Si
\[
m:=\min_{x\in[a,b]}f''(x),\qquad M_2:=\max_{x\in[a,b]}f''(x),
\]
entonces $m\le f''(\eta_j)\le M_2$ para todo $j$, y por promediar también $m\le\overline s\le M_2$.  Como $f''$ es continua en $[a,b]$, por el teorema del valor intermedio existe $\eta\in[a,b]$ tal que $f''(\eta)=\overline s$. Sustituyendo:
\[
E=-\frac{h^3}{12}\,M\,f''(\eta)=-\frac{(b-a)h^2}{12}\,f''(\eta).
\]
Como el error es no trivial salvo casos degenerados, podemos tomar $\eta\in(a,b)$.
\end{proof}
...
\end{theorem}
\begin{proof}
Particionamos $[a,b]$ en $x_j=a+jh$, $j=0,\dots,M$, con $h=(b-a)/M$.

\textbf{Paso 1: error local como integral de interpolación.}
En cada subintervalo $I_j=[x_j,x_{j+1}]$, sea $p_{1,j}$ el interpolante lineal de $f$ en los extremos:
\[
p_{1,j}(x)=f(x_j)\,\ell_{j,0}(x)+f(x_{j+1})\,\ell_{j,1}(x),
\]
con bases de Lagrange
\[
\ell_{j,0}(x)=\frac{x_{j+1}-x}{h},\qquad
\ell_{j,1}(x)=\frac{x-x_j}{h},\qquad
\ell_{j,0}(x)+\ell_{j,1}(x)=1.
\]
La regla del trapecio simple en $I_j$ es exactamente $\int_{x_j}^{x_{j+1}} p_{1,j}(x)\,dx$, luego
\[
E_j:=\int_{x_j}^{x_{j+1}} f(x)\,dx-T(f;x_j,x_{j+1})
=\int_{x_j}^{x_{j+1}}\bigl(f(x)-p_{1,j}(x)\bigr)\,dx.
\]

\textbf{Paso 2: ``intercalar'' $f(x)$ en la combinación convexa.}
Para $x\in I_j$:
\[
\begin{aligned}
f(x)-p_{1,j}(x)
&=f(x)-\ell_{j,0}(x)f(x_j)-\ell_{j,1}(x)f(x_{j+1})\\
&=\ell_{j,0}(x)\bigl(f(x)-f(x_j)\bigr)+\ell_{j,1}(x)\bigl(f(x)-f(x_{j+1})\bigr),
\end{aligned}
\]
donde usamos $\ell_{j,0}+\ell_{j,1}=1$ para escribir
$f(x)=\ell_{j,0}(x)f(x)+\ell_{j,1}(x)f(x)$ y restar término a término.

\textbf{Paso 3: cota por módulo de continuidad.}
Como $x,x_j,x_{j+1}\in I_j$ y $\operatorname{diam}(I_j)=h$,
\[
|f(x)-f(x_j)|\le\omega(f,h),\qquad |f(x)-f(x_{j+1})|\le\omega(f,h).
\]
Además $\ell_{j,0}(x),\ell_{j,1}(x)\ge 0$ en $I_j$. Entonces
\[
|f(x)-p_{1,j}(x)|
\le \ell_{j,0}(x)\omega(f,h)+\ell_{j,1}(x)\omega(f,h)
=\omega(f,h).
\]
Integrando en $I_j$:
\[
|E_j|\le\int_{x_j}^{x_{j+1}}\omega(f,h)\,dx=h\,\omega(f,h).
\]

\textbf{Paso 4: suma global.}
\[
\left|\int_a^b f(x)\,dx-T_M(f)\right|
=\left|\sum_{j=0}^{M-1}E_j\right|
\le\sum_{j=0}^{M-1}|E_j|
\le M h\,\omega(f,h)
=(b-a)\,\omega(f,h).
\]
Como $f$ es continua en $[a,b]$, es uniformemente continua, y por tanto $\omega(f,h)\to 0$ cuando $h\to 0$; se concluye $T_M(f)\to\int_a^b f$.
\end{proof}
...
\begin{theorem}[Cota mejorada para singularidad aislada]\label{prop:trapecio_singularidad}
...
\chapter{Teoría de Aproximación}\label{cap:teoria_aproximacion}
...
% En el capítulo de series de Fourier (Capítulo~\ref{cap:teoria_aproximacion}, Sección~\ref{sec:series_fourier_suaves}) vimos que la regularidad clásica de una función (por ejemplo, ser $C^n$) implica decaimiento de sus coeficientes de Fourier (Teorema~\ref{thm:decaimiento_fourier}).
