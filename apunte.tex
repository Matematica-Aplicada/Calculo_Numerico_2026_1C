...
\begin{proof}
Sea $T=\operatorname{supp}(\widehat{\mathbf{x}})$, con $|T|=t$. Por la fórmula de la IDFT,
\[
x_j=\frac1n\sum_{k=0}^{n-1}\widehat{x}_k\,\omega^{-jk}
=\frac1n\sum_{k\in T}\widehat{x}_k\,\omega^{-jk}.
\]
Tomando módulo y usando $|\omega^{-jk}|=1$:
\[
|x_j|\le \frac1n\sum_{k\in T}|\widehat{x}_k|.
\]
Aplicando Cauchy-Schwarz a la suma del lado derecho:
\[
\sum_{k\in T}|\widehat{x}_k|
...
\]
Si $S=\operatorname{supp}(\mathbf{x})$ y $|S|=s$, entonces
\[
\|\mathbf{x}\|_2^2
=\sum_{j\in S}|x_j|^2
\le s\left(\frac{\sqrt{t}}{n}\|\widehat{\mathbf{x}}\|_2\right)^2
=\frac{st}{n^2}\,\|\widehat{\mathbf{x}}\|_2^2.
\]
Por la unitariedad en nuestra convención ($F_n^*F_n=nI$), vale
\[
...
\]
Como $\mathbf{x}\neq 0$, tenemos $\|\mathbf{x}\|_2^2>0$, y por tanto $st\ge n$.
\end{proof}
