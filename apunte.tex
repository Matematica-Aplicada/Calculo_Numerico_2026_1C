...
\[
|x_{k+1}-x^*|\le C\,|x_k-x^*|^{\varphi},\qquad
\varphi=\frac{1+\sqrt{5}}{2}\approx 1.618.
\]
El orden de convergencia es el número áureo $\varphi$: superlineal pero sublineal
respecto del orden cuadrático de Newton.
\end{theorem}

\begin{proof}
Denotamos $e_k=x_k-x^*$.  Usando la expansión de Taylor $f(x_k)=f'(x^*)e_k+\tfrac12 f''(x^*)e_k^2+O(e_k^3)$
(y análogamente para $f(x_{k-1})$) y la fórmula de diferencias divididas
\[
\frac{f(x_k)-f(x_{k-1})}{x_k-x_{k-1}}=f'(x^*)+\tfrac12 f''(x^*)(e_k+e_{k-1})+O(e_k^2+e_{k-1}^2),
\]
se obtiene, tras simplificar los términos dominantes:
\[
e_{k+1}\;\approx\;\frac{f''(x^*)}{2f'(x^*)}\,e_k\,e_{k-1}.
\]
Sea $C_0=|f''(x^*)/(2f'(x^*))|$ y $M_k=C_0|e_k|$.  La relación anterior se convierte en
$M_{k+1}\approx M_k M_{k-1}$; tomando logaritmos con $u_k=\log M_k$:
\[
u_{k+1}=u_k+u_{k-1}.
\]
La ecuación característica de esta recurrencia lineal es $\lambda^2=\lambda+1$, con raíces
$\varphi=(1+\sqrt5)/2\approx1.618$ y $\hat\varphi=(1-\sqrt5)/2\approx-0.618$.
Si la sucesión converge ($u_k\to-\infty$), la componente en $\varphi^k$ domina y
$u_k\sim A\varphi^k$, lo que da $|e_k|\sim \exp(A\varphi^k)$.
En particular, $|e_{k+1}|\approx|e_k|^\varphi$, que es exactamente la cota del enunciado
(para una demostración rigurosa ver, e.g., Stoer-Bulirsch).
\end{proof}
