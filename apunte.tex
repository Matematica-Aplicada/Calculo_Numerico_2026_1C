...
Sea $n\in\mathbb N\cup\{0\}$ y sea $f\in C^n_{per}([0,2\pi];\mathbb C)$. Si $(c_k)_{k\in\mathbb Z}\subset\mathbb C$ satisface
\[
f=\sum_{k\in\mathbb Z} c_k e^{ikx}
\]
Donde la convergencia se entiende en norma $L^2$ (es decir, $\|f-S_N\|\to0$), entonces los coeficientes $c_k$ coinciden con los coeficientes de Fourier complejos de $f$:
\[
c_k=\hat f_k\qquad\text{para todo }k\in\mathbb Z.
\]
\end{theorem}

\begin{proof}
Para $N\ge |m|$, por ortogonalidad de las exponenciales complejas,
\[
(S_N,e_m)=\sum_{|k|\le N}c_k(e_k,e_m)=2\pi c_m.
\]
Además, la continuidad del producto interno respecto de la norma (Cauchy--Schwarz) da
\[
|(f,e_m)-(S_N,e_m)|=|(f-S_N,e_m)|
\le \|f-S_N\|_2\,\|e_m\|_2\xrightarrow[N\to\infty]{}0,
\]
de donde
\[
(f,e_m)=\lim_{N\to\infty}(S_N,e_m)=2\pi c_m.
\]
Por la definición de coeficiente de Fourier complejo,
\[
\hat f_m=\frac{(f,e_m)}{2\pi}=c_m.
\]
Como $m$ es arbitrario, vale para todo índice.
\end{proof}
