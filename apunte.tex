...
A diferencia del caso finito-dimensional, esta suma es en general infinita y surgen dos preguntas centrales. La primera es la convergencia: ¿en qué sentido tiende la serie completa a $f$? Y si la serie converge, la segunda pregunta es ¿Con qué velocidad lo hace? Equivalentemente, si retenemos sólo los primeros $N$ términos de la representación, ¿cuán grande es el error $\|f - \sum_{|k| \leq N} \hat{f}_k \phi_k\|$? Las respuestas dependen de la regularidad de $f$ y de la elección de la base, y su análisis es el hilo conductor de este capítulo.

\begin{remark}[Alcance analítico del capítulo]
En esta exposición trabajamos principalmente con funciones continuas (y, cuando hace falta, suaves) e integrales en intervalos acotados. Con este marco se pueden demostrar de manera elemental los resultados centrales para series de Fourier de funciones continuas y periódicas: definición de coeficientes, decaimiento por integración por partes, convergencia mediante núcleo de Dirichlet y estimaciones de truncamiento. La formulación en términos de espacios $L^2$, integral de Lebesgue y espacios de Hilbert no es necesaria para estos resultados básicos, pero sí proporciona la generalización natural a datos menos regulares y un marco abstracto unificado.
\end{remark}
...
Para funciones a valores complejos $f,g:[0,2\pi]\to\mathbb{C}$, la integral se define componente a componente: si $f=u+iv$ con $u,v$ reales, entonces
\[
\int_0^{2\pi}f\,dx = \int_0^{2\pi}u\,dx + i\int_0^{2\pi}v\,dx.
\]
Con esta convención, el espacio $C^n_{per}([0,2\pi];\mathbb{C})$ queda dotado del producto interno:
...
\begin{lemma}[Funciones continuas son cuadrado integrables]
Si $f\in C([a,b];\mathbb{C})$, entonces $f$ es cuadrado integrable en $[a,b]$, es decir,
\[
...
\begin{proof}
Como $f$ es continua en el compacto $[a,b]$, existe $M\ge 0$ tal que $|f(x)|\le M$ para todo $x\in[a,b]$. Luego
\[
0\le \int_a^b |f(x)|^2\,dx \le \int_a^b M^2\,dx = M^2(b-a) < \infty.
\]
\end{proof}
...
Sea $f:[0,2\pi]\to\mathbb{R}$ integrable. Sus \textbf{coeficientes de Fourier reales} son:
...
\end{equation}
La \textbf{suma parcial de Fourier real} de orden $N$ es:
...
Sea $\{\varphi_k\}$ una base ortogonal de $L^2_w(a,b)$, sea $f\in C([a,b])$ y sea $S_n f$ la proyección de orden $n$.  Entonces:
\begin{enumerate}
...
\chapter{Convergencia de Series de Fourier}

En el cuerpo principal de las notas (Capítulo~\ref{cap_fourier_generalizado}, Sección~\ref{sec:series_fourier_suaves}) ya establecimos la relación básica entre regularidad y decaimiento de coeficientes de Fourier. En particular, el Teorema~\ref{thm:decaimiento_fourier} y el Corolario~\ref{cor:decaimiento_O_nk} muestran que la regularidad $C^m$ implica decaimiento algebraico $|\hat f_k|=O(|k|^{-m})$, mientras que el Teorema~\ref{thm:converso_suavidad_decaimiento} da un converso útil en términos de sumabilidad ponderada de coeficientes.
...
\begin{theorem}[Parseval para funciones $C^1_{per}$]\label{thm:parseval_C1_per}
Sea $f\in C^1_{per}([0,2\pi])$ y sea
\[
S_Nf(x)=\sum_{|k|\le N}\hat f_k e^{ikx},
\qquad
...
\]
Si $p(x)=\sum_{|k|\le N}a_k e^{ikx}\in\mathcal T_N$, entonces
\[
\begin{aligned}
(f-S_Nf,p)
&=\sum_{|k|\le N}\overline{a_k}\,(f-S_Nf,e^{ikx})\\
&=\sum_{|k|\le N}\overline{a_k}\,\big((f,e^{ikx})-(S_Nf,e^{ikx})\big)=0,
\end{aligned}
\]
porque $(S_Nf,e^{ikx})=(f,e^{ikx})$ para $|k|\le N$ por definición de $\hat f_k$. Por lo tanto,
$f-S_Nf\perp\mathcal T_N$, es decir, $S_Nf$ es la proyección ortogonal de $f$ sobre $\mathcal T_N$.

Aplicando Pitágoras en esta descomposición ortogonal,
\[
\|f\|_{L^2}^2=\|S_Nf\|_{L^2}^2+\|f-S_Nf\|_{L^2}^2.
\]
Además, por ortogonalidad de la base exponencial,
\[
...
Como $f\in C^1_{per}$, por el Teorema~\ref{thm:convergencia_uniforme_fourier} (con $n=1$) se tiene $\|f-S_Nf\|_\infty\to 0$. En consecuencia,
\[
\|f-S_Nf\|_{L^2}\le \sqrt{2\pi}\,\|f-S_Nf\|_\infty\xrightarrow[N\to\infty]{}0.
\]
Pasando al límite en la igualdad anterior, obtenemos
\[
\|f\|_{L^2}^2=2\pi\sum_{k\in\mathbb Z}|\hat f_k|^2.
\]
\end{proof}
...
\begin{remark}[Qué aporta el teorema de convergencia $C^1$]
El Teorema~\ref{thm:conv_C1} garantiza que, para $f\in C^1_{per}$, la serie de Fourier converge a $f$. Sin embargo, por sí solo no da una tasa explícita del error $\|f-S_Nf\|_\infty$ en función de $N$. La cota cuantitativa del Teorema~\ref{thm:convergencia_uniforme_fourier} aporta justamente esa información de velocidad de convergencia.
\end{remark}
...
\begin{corollary}[Criterio práctico de suavidad por cota de decaimiento]
Sea $m\in\mathbb N\cup\{0\}$. Si
\[
|c_k|\le \frac{C}{(1+|k|)^{m+1+\varepsilon}}
\quad\text{con }\varepsilon>0,
\]
entonces la serie trigonométrica $\sum_{k\in\mathbb Z} c_k e^{ikx}$ define una función de clase $C^m_{per}$.
\end{corollary}

\begin{proof}
La hipótesis implica
\[
(1+|k|)^m|c_k|\le \frac{C}{(1+|k|)^{1+\varepsilon}},
\]
y como $\sum_k (1+|k|)^{-1-\varepsilon}<\infty$, se cumple la hipótesis del Teorema~\ref{thm:converso_suavidad_decaimiento}.
\end{proof}

\begin{remark}[Resultados finos: convergencia y derivación término a término]
Los resultados demostrados en esta sección cubren el marco $C^n_{per}$, pero existen teoremas más ajustados:
\begin{itemize}
\item \textbf{Convergencia de la serie de Fourier:}
\begin{itemize}
...
\end{itemize}
\item \textbf{Hipótesis mínimas naturales para derivar término a término:}
\begin{itemize}
\item Para la identidad de coeficientes $\widehat{f'}_k=ik\hat f_k$ basta que $f$ sea absolutamente continua, $2\pi$-periódica y $f'\in L^1([0,2\pi])$.
\item Para justificar rigurosamente la derivación de la serie, alcanza el criterio del Lema~\ref{lem:criterio_funcional_derivacion}: convergencia en un punto de $S_Nf$ y convergencia uniforme de $(S_Nf)'$ en $[0,2\pi]$. En particular, la hipótesis $f\in C^2_{per}$ del Teorema~\ref{thm:derivacion_termino_termino_fourier} garantiza este criterio en forma directa.
\end{itemize}
\end{itemize}
\end{remark}
