...
En este capítulo queremos obtener representaciones de funciones en bases de funciones, extendiendo la fórmula de proyección \eqref{proyeccion_ortogonal_appendix} utilizada en el capítulo \ref{cap_aln} para el estudio del problema de la mejor aproximación lineal en el sentido de mínimos cuadrados, a espacios de dimensión infinita. La idea es la misma que en el caso finito-dimensional: si $\{\phi_k\}_{k \in \mathcal{I}}$ es una familia ortonormal en un espacio de funciones dotado de un producto interno apropiado, podemos definir una representación
\[
f \;\sim\; \sum_{k \in \mathcal{I}} (f, \phi_k) \phi_k,
\]
que llamamos \emph{serie} de $f$ en la base $\{\phi_k\}$. Los coeficientes de esta serie son las coordenadas de $f$ en la base $\{\phi_k\}$, y se denominan \textit{coeficientes de Fourier generalizados} de $f$ (en la base dada).

A diferencia del caso finito-dimensional, esta suma es en general infinita y surgen dos preguntas centrales. La primera es la convergencia: ¿en qué sentido tiende la serie completa a $f$? Y si la serie converge, la segunda pregunta es ¿Con qué velocidad lo hace? Equivalentemente, si retenemos sólo los primeros $N$ términos de la representación, ¿cuán grande es el error $\|f - S_N(f)\|$, donde $S_N(f)$ es la suma parcial de los primeros $N$ términos? Como veremos, las respuestas dependen fundamentalmente de la regularidad de $f$ y de la elección de la base, y su análisis es el hilo conductor de este capítulo.

\section{Series de Fourier de funciones suaves periódicas}\label{sec:series_fourier_suaves}

La teoría de series de Fourier es un área muy rica y profunda, con una historia que se remonta a los trabajos de Joseph Fourier a comienzos del siglo XIX. Fourier introdujo estas expansiones en su estudio de la conducción del calor, modelada por
\[
u_t = \alpha u_{xx},
\]
donde $u(x,t)$ representa la temperatura en el punto $x$ y en el tiempo $t$. Mediante separación de variables, obtuvo soluciones en términos de senos y cosenos y postuló que funciones muy generales podían representarse por series trigonométricas. Esa intuición fue extraordinariamente fecunda, aunque varias afirmaciones iniciales eran todavía imprecisas, y algunas formulaciones resultaron ser falsas sin hipótesis adicionales.

El desarrollo riguroso de la convergencia de series de Fourier (condiciones de convergencia puntual, convergencia en distintas normas, comportamiento en discontinuidades, etc.) tomó muchas décadas y consolidó una parte central del análisis moderno. En este curso nos concentramos en las consecuencias y aplicaciones numéricas de esta teoría. Trabajaremos principalmente con funciones periódicas suficientemente suaves (con $n$ derivadas continuas, $n \ge 0$), donde se pueden demostrar de manera relativamente elemental los resultados centrales: definición de coeficientes, estimaciones de decaimiento, convergencia puntual y uniforme, la identidad de Parseval y cotas de error de truncamiento. Por una cuestión de espacio, deferiremos varias de estas demostraciones al Apéndice \ref{app_convergencia_fourier}, que incluye un compendio de resultados de convergencia que cubre funciones al menos continuas a trozos (integrables de Riemann), sin requerir el uso de la integral de Lebesgue.

En esta sección trabajamos con funciones de variable real y valores complejos,
\[
f:[0,2\pi]\to\mathbb{C},\qquad f=u+iv,
\]
donde $u,v:[0,2\pi]\to\mathbb{R}$.
...
\subsection{Producto interno, norma cuadrática y cuadrado integrabilidad}

El espacio $C^n_{per}([0,2\pi];\mathbb{C})$ puede dotarse del producto interno:
\begin{equation}\label{prod_cont}
(f,g) = \int_0^{2\pi} f(x)\,\overline{g(x)}\,dx,
\end{equation}
donde $\overline{g(x)}$ denota el conjugado complejo.

\begin{definition}[Norma cuadrática]
La norma asociada al producto interno \eqref{prod_cont} se define por
\[
\|f\|_2 := \sqrt{(f,f)} = \left(\int_0^{2\pi}|f(x)|^2\,dx\right)^{1/2}.
\]
\end{definition}

\subsection{La base exponencial compleja}

Para cada $N\in\mathbb{N}$, la base de exponenciales complejas es:
\begin{equation}\label{bn_complex}
B_N^{\mathbb{C}} = \left\{ e^{ikx} : k\in\mathbb{Z},\;|k|\le N \right\} \subset C^\infty_{per}([0,2\pi]).
\end{equation}

\begin{theorem}[Ortogonalidad de la base exponencial]\label{thm:ortog_complex}
Para $k, l\in\mathbb{Z}$:
\[
(e^{ikx},\,e^{ilx}) = \int_0^{2\pi}e^{i(k-l)x}\,dx =
\begin{cases} 2\pi & \text{si }k=l,\\ 0 & \text{si }k\ne l. \end{cases}
\]
En particular, $\|e^{ikx}\|^2 = 2\pi$ para todo $k\in\mathbb{Z}$.
\end{theorem}

\begin{proof}
Si $k=l$: $\int_0^{2\pi}1\,dx = 2\pi$. Si $k\ne l$, sea $m=k-l\ne 0$:
\[
...
Sea $f\in C([0,2\pi];\mathbb{C})$. Sus coeficientes de Fourier complejos (respecto de la familia $\{e^{ikx}\}_{k\in\mathbb{Z}}$) se definen por la fórmula de proyección~\eqref{proyeccion_ortogonal_appendix}:
\begin{equation}\label{coef_fourier_complex}
\hat{f}_k = \frac{(f,\,e^{ikx})}{\|e^{ikx}\|^2}
...
\end{equation}
\end{definition}
Como $S_Nf$ es la proyección ortogonal de $f$ sobre $\langle B_N^{\mathbb{C}}\rangle$, es el polinomio trigonométrico de grado $\le N$ más cercano a $f$ en norma cuadrática.

\begin{theorem}[Propiedad de mejor aproximación]\label{thm:mejor_aprox}
Sea $f\in C([0,2\pi];\mathbb{C})$ y $N\ge 0$. Para todo polinomio trigonométrico $p=\sum_{|k|\le N}c_k e^{ikx}$:
\[
\|f - S_N f\|_2 \le \|f - p\|_2,
\]
donde $\|\cdot\|_2$ es la norma cuadrática asociada a \eqref{prod_cont}.
\end{theorem}

\begin{proof}
Escribimos $p = S_N f + q$ con $q = p - S_N f \in \langle B_N^{\mathbb{C}}\rangle$. Como $f - S_N f \perp \langle B_N^{\mathbb{C}}\rangle$ por construcción de $S_Nf$ como proyección ortogonal, la identidad de Pitágoras da:
\[
\|f-p\|_2^2 = \|f-S_N f\|_2^2 + \|q\|_2^2 \ge \|f-S_N f\|_2^2. \qedhere
\]
\end{proof}

\begin{theorem}[Equivalencia entre densidad, convergencia en $L^2$ y Parseval]
Sea $f\in C([0,2\pi];\mathbb C)$. Son equivalentes:
\begin{enumerate}
\item Para todo $\varepsilon>0$ existe un polinomio trigonométrico $p$ tal que $\|f-p\|_2<\varepsilon$.
\item $\|f-S_Nf\|_2\xrightarrow[N\to\infty]{}0$.
\item Identidad de Parseval: \[\|f\|_2^2 = 2\pi\sum_{k\in\mathbb Z}|\hat f_k|^2.\]
\end{enumerate}
\end{theorem}

\begin{proof}
\textbf{(1) $\Rightarrow$ (2).} Sea $\varepsilon>0$ y tome $p(x)=\sum_{|k|\le m}c_k e^{ikx}$ con $\|f-p\|_2<\varepsilon$. Para $N\ge m$, como $p\in\mathcal T_N$, por el Teorema~\ref{thm:mejor_aprox}:
\[
\|f-S_Nf\|_2\le \|f-p\|_2<\varepsilon.
\]
Luego $\|f-S_Nf\|_2\to 0$.

\textbf{(2) $\Rightarrow$ (3).} Para cada $N$, por ortogonalidad de los exponentiales:
\[
\|f\|_2^2 = 2\pi\sum_{|k|\le N}|\hat f_k|^2 + \|f-S_Nf\|_2^2.
\]
Si $\|f-S_Nf\|_2\to 0$, al pasar al límite se obtiene Parseval.

\textbf{(3) $\Rightarrow$ (2).} De la misma identidad finita,
\[
\|f-S_Nf\|_2^2 = \|f\|_2^2 - 2\pi\sum_{|k|\le N}|\hat f_k|^2.
\]
Bajo (3), el segundo término converge a $\|f\|_2^2$, luego $\|f-S_Nf\|_2\to 0$.

Finalmente, (2) implica (1) tomando $p=S_Nf$ para $N$ grande.
\end{proof}

\begin{corollary}[Identidad de Parseval para el error de truncamiento]\label{cor:parseval_error_truncamiento}
Bajo las hipótesis equivalentes del teorema anterior, para todo $N\ge 0$ se cumple
\begin{equation}\label{eq:parseval_error_truncamiento}
\|f-S_Nf\|_2^2 = 2\pi\sum_{|k|>N}|\hat f_k|^2.
\end{equation}
\end{corollary}

\begin{proof}
Para cada $N$, la identidad finita del teorema anterior da
\[
\|f\|_2^2 = 2\pi\sum_{|k|\le N}|\hat f_k|^2 + \|f-S_Nf\|_2^2.
\]
Usando Parseval,
\[
\|f\|_2^2 = 2\pi\sum_{k\in\mathbb Z}|\hat f_k|^2,
\]
y restando término a término se obtiene
\[
\|f-S_Nf\|_2^2 = 2\pi\sum_{|k|>N}|\hat f_k|^2.
\]
\end{proof}

En particular, admitiendo la densidad trigonométrica en $L^2$ del Apéndice~\ref{app_convergencia_fourier} (Teorema~\ref{thm:densidad_trig_L2_riemann}), las tres propiedades valen para toda $f$ continua a trozos.
...
\]
Es decir, la regla del trapecio periódica integra \textbf{exactamente} a $e^{i\ell x}$ siempre que $\ell$ no sea múltiplo no nulo de $M$. En particular, tomando $M=2N+1$ se obtiene exactitud para
\[
0\le k<2N+1\quad(\text{equivalentemente }k\le 2N),
\]
y no para $k=2N+1$.
\end{lemma}
\begin{proof}
...
Para poder identificar el resto con una cola infinita de coeficientes hace falta, además de ortogonalidad, una hipótesis de densidad/completitud. En el caso de polinomios ortogonales sobre intervalos compactos, esa densidad se obtiene con Stone-Weierstrass.
