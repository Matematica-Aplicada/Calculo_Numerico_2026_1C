...
\]
con coeficientes $a_k = \frac{2}{\pi}\int_0^{\pi} g(\theta)\cos(k\theta)\,d\theta$.  Cuando volvemos a la variable original $x = \cos\theta$, se obtienen los polinomios de Chebyshev $T_k(x) = \cos(k\arccos x)$, y la serie de Fourier en cosenos se traduce en la serie de Chebyshev.
...
La razón por la que aparecen sólo cosenos es la siguiente. Como $g$ es par, en su serie de Fourier real
\[
g(\theta)=\frac{a_0}{2}+\sum_{k=1}^{\infty}\big(a_k\cos(k\theta)+b_k\sin(k\theta)\big),
\]
los coeficientes seno satisfacen
\[
b_k=\frac{1}{\pi}\int_{-\pi}^{\pi} g(\theta)\sin(k\theta)\,d\theta=0,
\]
porque $g$ es par, $\sin(k\theta)$ es impar y el producto es impar. Luego la expansión de Fourier de $g$ contiene únicamente términos coseno.
