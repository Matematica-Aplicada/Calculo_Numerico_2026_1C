...
\begin{proof}
Sea $e^n=u_h^n-U^n$. De la definición de $\tau_h^n$ y el esquema:
$e^{n+1}=S_he^n+\Delta t\,\tau_h^n$, $e^0=0$.

\textbf{($\Leftarrow$)}  Iterando: $e^n=\Delta t\sum_{m=0}^{n-1}S_h^{n-1-m}\tau_h^m$. Tomando norma,
\[
\|e^n\|_{2,h}\le C_T\,T\max_{m}\|\tau_h^m\|_{2,h}\xrightarrow{h,\Delta t\to0}0.
\]

\textbf{($\Rightarrow$)}  En el caso periódico lineal circulante la implicación inversa se prueba modo a modo.  Sea $v^{(k)}$ el $k$-ésimo modo discreto (norma 1). Como $S_h$ es circulante, $S_h^nv^{(k)}=G_h(\theta_k)^nv^{(k)}$. El problema continuo tiene solución modal $e^{\lambda(\theta_k)t}e^{i\theta_k x}$ con $|e^{\lambda(\theta_k)t}|\le e^{\omega T}$ (bien-puesta uniforme en $k$). La convergencia aplicada al dato modal da
\[
|G_h(\theta_k)^n-e^{\lambda(\theta_k)t_n}|\to0,
\]
luego para $h$ suficientemente fino, $|G_h(\theta_k)|^n\le e^{\omega T}+1$. Maximizando en $k$: $\|S_h^n\|\le e^{\omega T}+1$, que es la estabilidad.
\end{proof}

\begin{remark}
Fuera del caso periódico lineal, Fourier no diagonaliza el operador completo: el control del borde requiere análisis adicional (como en el Teorema~\ref{thm:dirichlet_reaction_new} para Dirichlet).
\end{remark}
