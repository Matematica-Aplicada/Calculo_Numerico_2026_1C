...
Sea $A\in\mathbb{C}^{m\times n}$ con $m\ge n$ y $\operatorname{rank}(A)=n$, y defina
\[
H:=AA^+=A(A^*A)^{-1}A^*\in\mathbb{C}^{m\times m}.
\]
Entonces:
\begin{enumerate}[(i)]
\item $H^*=H$ y $H^2=H$.
...
\begin{proof}
De la definición, usando $(A^*A)^{-1}$ hermítica, se obtiene $H^*=H$. Además,
\[
H^2=A(A^*A)^{-1}A^*A(A^*A)^{-1}A^*=A(A^*A)^{-1}A^*=H.
\]
Así, $H$ es proyector hermítico, luego proyección ortogonal sobre su imagen. Como $H=AA^+$, se tiene $\operatorname{Im}(H)=\operatorname{Col}(A)$. También,
\[
\hat{d}=A\hat{x}=A(A^+d)=Hd.
\]
Finalmente, para el vector canónico $e_i$,
\[
h_{ii}=e_i^*He_i=(He_i)^*He_i=\|He_i\|_2^2\ge0,
\]
y como $H$ es proyector ortogonal, $\|H\|_2=1$, de modo que
\[
h_{ii}=\|He_i\|_2^2\le \|H\|_2^2\|e_i\|_2^2\le 1.
\]
\end{proof}
...
\]
y por ortonormalidad de $\{u_i\}$ y $\{v_i\}$:
\[
\|\delta x\|_2^2
...
\begin{remark}
El Corolario~\ref{cor:inv_normal} se aplica en particular a las matrices hermitianas ($A = A^*$) y a las unitarias ($A^*A = I$), que son casos particulares de matrices normales. Para las unitarias, todos los autovalores tienen módulo 1, de modo que $\|A\|_2 = 1$ y $\kappa_2(A) = 1$.

La hipótesis de normalidad es esencial. Por ejemplo, la matriz triangular superior
\[
A = \begin{pmatrix} 0 & 1 \\ 0 & 0 \end{pmatrix}
\]
tiene $\rho(A) = 0$ (su único autovalor es $\lambda = 0$), pero $\|A\|_2 = 1$ (pues $\sigma_1 = 1$). Este ejemplo muestra que para matrices no normales, la desigualdad $\rho(A) \leq \|A\|_2$ del Teorema~\ref{thm:radio_espectral_norma} puede ser estricta.
\end{remark}
