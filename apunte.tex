...
\subsubsection*{Espaciado entre números consecutivos}

El épsilon de máquina determina la resolución \textit{relativa} del sistema.  Para entender esto, observemos cómo están espaciados los números de máquina.

Sea $x = d_0.d_1\cdots d_{t-1} \times \beta^e$ un número de máquina normalizado.  Todos los números de máquina con el mismo exponente $e$ difieren solo en los dígitos de la mantisa.  La distancia entre dos números consecutivos con exponente $e$ es
\[
\Delta_e = \beta^{-(t-1)} \times \beta^e = \beta^{e-t+1}.
\]
Nótese que este espaciado \textbf{depende del exponente} $e$: los números de máquina se hacen más densos cerca de cero y más espaciados lejos del origen. Sin embargo, el espaciado \textit{relativo} es aproximadamente uniforme.  En efecto, como los números con exponente $e$ satisfacen $\beta^{e-1}\le|x|<\beta^e$\footnote{Para $e=e_{\min}$ el rango es $\beta^{e_{\min}-1}\le |x|<\beta^{e_{\min}}$.  Ignoramos los subnormales en este análisis.}, el espaciado relativo satisface:
\[
\frac{\Delta_e}{|x|} \;\le\; \frac{\beta^{e-t+1}}{\beta^{e-1}} \;=\; \beta^{2-t}.
\]
Pero $\epsilon_{\text{mach}}=\beta^{1-t}$, así que $\Delta_e/|x| \le \beta\cdot\epsilon_{\text{mach}}$ para todo $x$ normalizado. En el caso particular $\beta=2$, esto da $\Delta_e/|x|\le 2\epsilon_{\text{mach}}$, y la cota fina (que probaremos a continuación) muestra que al redondear al más cercano, el error relativo no supera $\epsilon_{\text{mach}}/2$.
...
La propiedad más importante del redondeo es la siguiente cota de error relativo.
...
\begin{proof}
Sea $x\neq 0$ un número real en el rango normalizado.  Entonces $x$ se encuentra entre dos números de máquina consecutivos.  Escribimos
\[
x = \pm\, d_0.d_1\cdots d_{t-1}\,d_t\,d_{t+1}\cdots\;\times\;\beta^e,
\]
con $d_0\neq 0$, donde los dígitos $d_t, d_{t+1},\ldots$ son los que no caben en la mantisa.

Los dos números de máquina consecutivos que encierran a $|x|$ son:
\begin{align*}
x^- &= d_0.d_1\cdots d_{t-1}\;\times\;\beta^e,\\
x^+ &= x^- + \beta^{e-(t-1)}.
\end{align*}
El redondeo al más cercano satisface
\[
|\operatorname{fl}(x)-x| \;\le\; \tfrac{1}{2}\,(x^+-x^-) \;=\; \tfrac{1}{2}\,\beta^{e-(t-1)} \;=\;\tfrac{1}{2}\,\beta^{e-t+1}.
\]
Como $x$ es normalizado con exponente $e$, la mantisa satisface $d_0\ge 1$, y por lo tanto
\[
|x| \;\ge\; 1.0\cdots 0 \times \beta^e \;=\;\beta^e.
\]
Dividiendo:
\[
...
La formulación $\operatorname{fl}(x)=x(1+\delta)$ con $|\delta|\le u$ es particularmente útil para el \textbf{análisis de errores de redondeo}: permite propagar errores a través de secuencias de operaciones aritméticas.  El estándar IEEE~754 garantiza además que cada operación aritmética elemental ($+,-,\times,/$) se realiza con esta misma precisión: si $a,b\in\mathbb{F}$, entonces
\[
\operatorname{fl}(a\mathbin{\texttt{op}} b) = (a\mathbin{\texttt{op}} b)(1+\delta), \qquad |\delta|\le u,
\]
donde $\texttt{op}\in\{+,-,\times,/\}$.
