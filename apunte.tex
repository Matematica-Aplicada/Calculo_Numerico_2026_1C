...
\]
que explota toda la exactitud disponible de la regla gaussiana.
...
\begin{proof}
Como $E_n(p) = 0$ para todo $p \in \Pi_{2n-1}$, y $S_{2n-1}f \in \Pi_{2n-1}$, tenemos
\[
E_n(f) = E_n(f - S_{2n-1}f).
\]
Sea $g = f - S_{2n-1}f$.  Entonces:
\[
...
\[
\left|\int_a^b g\,w\right| \leq \int_a^b |g|\,w \leq \|g\|_{\infty}\int_a^b w = \mu_0\,\|g\|_{\infty}.
\]

\textbf{Término de cuadratura.}\; Como $w_k>0$:
\[
\left|\sum_{k=1}^n w_k\,g(x_k)\right| \leq \sum_{k=1}^n w_k\,|g(x_k)| \leq \|g\|_{\infty}\sum_{k=1}^n w_k = \mu_0\,\|g\|_{\infty}.
\]

\textbf{Conclusión:}
\[
|E_n(f)| = |E_n(g)| \leq 2\mu_0\,\|g\|_{\infty} = 2\mu_0\,\|f - S_{2n-1}f\|_{\infty}. \qedhere
\]
\end{proof}
...
En problemas de contorno no se avanza paso a paso en el tiempo: la discretización espacial conduce a un sistema lineal
\[
  A^h u^h = F^h,
\]
y la estabilidad se estudia en términos de la invertibilidad de $A^h$ y del tamaño de $(A^h)^{-1}$.
...
\begin{proof}
Taylor alrededor de $x_j$ da
\[
  u(x_{j\pm1})=u(x_j)\pm h u'(x_j)+\frac{h^2}{2}u''(x_j)\pm\frac{h^3}{6}u^{(3)}(x_j)+\frac{h^4}{24}u^{(4)}(\xi_j^{\pm}).
\]
Sumando ambas expresiones se cancelan los términos impares y queda
\[
  u(x_{j-1})-2u(x_j)+u(x_{j+1})
  =h^2u''(x_j)+\frac{h^4}{24}\bigl(u^{(4)}(\xi_j^-)+u^{(4)}(\xi_j^+)\bigr).
\]
Dividiendo por $h^2$ y absorbiendo el promedio de las derivadas cuartas en un punto intermedio $\xi_j$ se obtiene la fórmula.
\end{proof}

\subsection{Dirichlet con reacción: esquema centrado y convergencia}

Sea
\[
-u''(x)+\sigma u(x)=f(x),\quad x\in(0,1),\quad u(0)=\alpha,\ u(1)=\beta,\ \sigma>0.
\]
Tomamos la malla uniforme $x_j=jh$, $j=0,1,\dots,m+1$, con $h=1/(m+1)$, y buscamos aproximaciones $u_j\approx u(x_j)$ para los nodos interiores $j=1,\dots,m$. La discretización centrada interior es
\[
-\frac{u_{j-1}-2u_j+u_{j+1}}{h^2}+\sigma u_j=f(x_j),\qquad j=1,\dots,m.
\]
Las condiciones de borde se imponen fijando directamente $u_0=\alpha$ y $u_{m+1}=\beta$. Por lo tanto, en las ecuaciones para $j=1$ y $j=m$ esos valores se trasladan al lado derecho. El sistema para el vector incógnita
\[
  u^h=(u_1,\dots,u_m)^T
\]
toma la forma
...
\begin{theorem}[Dirichlet con reacción: consistencia + estabilidad + convergencia]\label{thm:dirichlet_reaction_new}
Si $u\in C^4([0,1])$, entonces el esquema centrado anterior es consistente de orden 2, estable en norma 2 y converge con
\[
  \|E^h\|_{2,h}\le Ch^2.
\]
\end{theorem}
...
Para la estabilidad, observamos que $A^h$ es una matriz real simétrica; en consecuencia, todos sus autovalores son reales. Además, cada fila tiene centro
\[
...
\[
  r_j\le \frac{2}{h^2}.
\]
Por el teorema de Gershgorin, todo autovalor $\lambda$ de $A^h$ pertenece al intervalo
\[
...
\]
que es exactamente la estabilidad en norma 2.
