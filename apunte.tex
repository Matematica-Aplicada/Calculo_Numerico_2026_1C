...
En los capítulos anteriores desarrollamos dos grandes herramientas para construir aproximaciones. En el capítulo de Álgebra Lineal Numérica estudiamos la mejor aproximación de un vector por elementos de un subespacio, en el sentido de mínimos cuadrados. La fórmula de proyección ortogonal \eqref{proyeccion_ortogonal_appendix} nos permitió expresar esta aproximación de manera explícita en términos de una base ortogonal del subespacio. En el capítulo de Interpolación vimos cómo construir polinomios que pasan exactamente por un conjunto de puntos dados, utilizando para ello diferentes bases de polinomios, como la base monomial, la base de Lagrage, y en el caso complejo la base de los modos de Fourier discretos.

En este capítulo queremos dar un paso más: obtener representaciones de funciones en bases de funciones, extendiendo la fórmula de proyección \eqref{proyeccion_ortogonal_appendix} a espacios de dimensión infinita. La idea es la misma que en el caso finito-dimensional: si $\{\phi_k\}_{k \in \mathcal{I}}$ es una familia ortogonal en un espacio de funciones dotado de un producto interno apropiado, los coeficientes $\hat{f}_k = (f, \phi_k)/\|\phi_k\|^2$ definen una representación
\[
f \;\sim\; \sum_{k \in \mathcal{I}} \hat{f}_k\, \phi_k,
\]
que llamamos \emph{serie} de $f$ en la base $\{\phi_k\}$.
...
Para todo $n \ge 1$, el espacio de funciones $n$ veces continuamente diferenciables y periódicas se define como:
$$C^n_{per}([0,2\pi]) = \left\lbrace f \in C^n([0,2\pi]) : f^{(p)}(0) = f^{(p)}(2\pi) \mbox{ para todo } 0\le p\le n-1 \right\rbrace.$$
La condición de periodicidad hasta el orden $n-1$ garantiza que la extensión de $f$ por periodicidad a todo $\mathbb{R}$ es de clase $C^{n-1}$: en particular $f$ es continua y periódica, y para $n \ge 2$ también lo es~$f'$, y así sucesivamente.

\subsection{La base exponencial compleja}

Para funciones a valores complejos $f,g:[0,2\pi]\to\mathbb{C}$, la integral se define componente a componente: si $f=u+iv$ con $u,v$ reales, entonces $\int_0^{2\pi}f\,dx = \int_0^{2\pi}u\,dx + i\int_0^{2\pi}v\,dx$. Con esta convención, el espacio $C^n_{per}([0,2\pi])$ queda dotado del producto interno:
\begin{equation}\label{prod_cont}
(f,g) = \int_0^{2\pi} f(x)\,\overline{g(x)}\,dx,
\end{equation}
donde $\overline{g(x)}$ denota el conjugado complejo. Para cada $N\in\mathbb{N}$, la base de exponenciales complejas es:
...
Los \textbf{coeficientes de Fourier} de $f$ son sus coordenadas en $B_N^{\mathbb{C}}$ según la fórmula de proyección~\eqref{proyeccion_ortogonal_appendix}:
\begin{equation}\label{coef_fourier_complex}
\hat{f}_k = \frac{(f,\,e^{ikx})}{\|e^{ikx}\|^2} = \frac{1}{2\pi}\int_0^{2\pi}f(x)\,e^{-ikx}\,dx, \qquad k\in\mathbb{Z}.
\end{equation}
La \textbf{suma parcial de Fourier} de orden $N$ es $S_Nf(x) = \sum_{|k|\le N}\hat{f}_k e^{ikx}$, y la \textbf{serie de Fourier} de $f$:
...
Sea $f\in L^2([0,2\pi];\mathbb{C})$ y $N\ge 0$. Para todo polinomio trigonométrico $p=\sum_{|k|\le N}c_k e^{ikx}$:
\[
\|f - S_N f\|_2 \le \|f - p\|_2,
\]
donde $\|h\|_2 = \bigl(\int_0^{2\pi}|h(x)|^2\,dx\bigr)^{1/2}$.
\end{theorem}
...
En este capítulo utilizamos $i$ para la unidad imaginaria ($i^2=-1$), siguiendo la convención matemática estándar. En ingeniería eléctrica y procesamiento de señales se usa a veces la notación $j$; ambas son equivalentes: $i\equiv j$.
\end{remark}
...
\begin{remark}
La ortogonalidad del conjunto $\{1,\cos(x),\sin(x),\ldots,\cos(Nx),\sin(Nx)\}$ en $L^2([0,2\pi])$ se obtiene sin verificación adicional: es una consecuencia del Teorema~\ref{thm:ortog_complex} y del cambio de base \eqref{eq:grouping}. El Teorema~\ref{thm:mejor_aprox} se extiende también al caso real: para $f$ real, $S_N(f)$ es el polinomio trigonométrico real de grado $\le N$ más cercano a $f$ en $L^2([0,2\pi])$.
\end{remark}
...
En el contexto periódico de Fourier, la misma idea se itera y los términos de borde se anulan exactamente.
...
Los resultados anteriores sobre decaimiento permiten organizar la convergencia en dos pasos: cota elemental del truncado y convergencia para $C^1_{per}$ mediante núcleo de Dirichlet y Riemann-Lebesgue.

\begin{lemma}[Cota elemental de truncamiento]\label{lem:fourier_abs_unif}\label{prop:fourier_rate_naive}
Sea $f \in C^n_{per}([0,2\pi])$ con $n\ge 1$. Para todo $N\ge 1$:
\[
\sum_{|k|>N}|\hat f_k| \le \frac{\|f^{(n)}\|_{L^2}}{\sqrt{\pi(2n-1)}}\,\frac{1}{N^{n-1/2}}.
\]
\end{lemma}

\begin{proof}
Para $k\neq 0$, el Teorema~\ref{thm:decaimiento_fourier} da $\hat f_k=\widehat{f^{(n)}}_k/(ik)^n$. Por Cauchy-Schwarz,
\[
\sum_{|k|>N}|\hat f_k|=\sum_{|k|>N}\frac{|\widehat{f^{(n)}}_k|}{|k|^n}
\le \Bigl(\sum_{|k|>N}|\widehat{f^{(n)}}_k|^2\Bigr)^{1/2}
...
\[
\sum_{|k|>N}|\hat f_k|
\le \sqrt{\frac{\|f^{(n)}\|_{L^2}^2}{2\pi}\cdot\frac{2}{(2n-1)N^{2n-1}}}
=\frac{\|f^{(n)}\|_{L^2}}{\sqrt{\pi(2n-1)}}\,\frac{1}{N^{n-1/2}}.
\]
La segunda desigualdad sigue de $\left|\sum_{|k|>N}\hat f_k e^{ikx}\right|\le \sum_{|k|>N}|\hat f_k|$.
\end{proof}
...
\begin{theorem}[Convergencia para funciones $C^1$]\label{thm:conv_C1}
...
\begin{proof}
Como $n\ge 1$, se tiene $f\in C^1_{per}$ y por el Teorema~\ref{thm:conv_C1} la serie de Fourier converge uniformemente a $f$. Luego
\[
\|f-S_Nf\|_\infty
=\left\|\sum_{|k|>N}\hat f_k e^{ikx}\right\|_\infty
\le \sum_{|k|>N}|\hat f_k|,
\]
y la cota final es la del Lema~\ref{lem:fourier_abs_unif}.
\end{proof}
