...
\]
que coincide con el valor en el nodo identificado $x=1$.
...
\]
Interpretación: en cada paso de tiempo, la componente de frecuencia $k$ queda multiplicada por $G_h(\theta_k)$. Tras $n$ pasos queda multiplicada por $G_h(\theta_k)^n$, por lo que la estabilidad uniforme hasta tiempo $T$ exige controlar estas potencias para todo $n\Delta t\le T$.
...
\[
\text{estabilidad de Lax--Richtmyer en }[0,T]
\iff
\sup_h\sup_{n\Delta t\le T}\max_k |G_h(\theta_k)|^n\le C_T.
\]
Además, esto es equivalente a la existencia de $\omega\in\mathbb{R}$ tal que, para $h$ suficientemente pequeño,
\[
\max_k|G_h(\theta_k)|\le e^{\omega\Delta t}
\quad\Longleftrightarrow\quad
\max_k|G_h(\theta_k)|=1+O(\Delta t).
\]
\end{proposition}

\begin{proof}
$S_h=F_h^{-1}\operatorname{diag}(G_h(\theta_k))F_h$ con $F_h$ unitario en $\ell^2_h$ (Teorema~\ref{thm:dft_unitary}). Por lo tanto,
\[
S_h^n=F_h^{-1}\operatorname{diag}(G_h(\theta_k)^n)F_h,
\]
y al tomar norma operador en $\ell_h^2$ se obtiene
\[
\|S_h^n\|_{2,h\to2,h}=\max_k|G_h(\theta_k)|^n.
\]
La equivalencia con Lax--Richtmyer en $[0,T]$ es inmediata al sustituir esta identidad en la definición
\(
\|S_h^n\|_{2,h\to2,h}\le C_T
\) para todo $n\Delta t\le T$.

Para la equivalencia con la cota por paso, sea $M_h:=\max_k|G_h(\theta_k)|$.

Si $M_h\le e^{\omega\Delta t}$, entonces para todo $n\Delta t\le T$:
\[
\|S_h^n\|_{2,h\to2,h}=M_h^n\le e^{\omega n\Delta t}\le e^{\omega T},
\]
y el esquema es estable.

Recíprocamente, si el esquema es estable en $[0,T]$, entonces existe $C_T$ con
\[
M_h^n\le C_T\quad\forall\,n\Delta t\le T.
\]
Tomando $n=N_h:=\lfloor T/\Delta t\rfloor$:
\[
M_h\le C_T^{1/N_h}=\exp\!\left(\frac{\log C_T}{N_h}\right).
\]
Para $\Delta t$ suficientemente pequeño, $N_h\ge T/(2\Delta t)$, luego
\[
M_h\le \exp\!\left(\frac{2\log C_T}{T}\,\Delta t\right)=e^{\omega\Delta t},
\]
...
Primero,
\[
|x_{j+1}-x_j|=|g(x_j)-g(x_{j-1})|\le q|x_j-x_{j-1}|\le\cdots\le q^j|x_1-x_0|.
\]
Luego, para $n>m$, por desigualdad triangular:
\[
|x_n-x_m|\le\sum_{j=m}^{n-1}|x_{j+1}-x_j|
           \le\sum_{j=m}^{n-1}q^j|x_1-x_0|
           =\left(\sum_{j=m}^{n-1}q^j\right)|x_1-x_0|.
\]
Para la suma de potencias, factorizamos $q^m$ y reindexamos:
\[
\sum_{j=m}^{n-1}q^j
=q^m\sum_{j=m}^{n-1}q^{j-m}
=q^m\sum_{r=0}^{n-1-m}q^r.
\]
Resultado auxiliar (fórmula geométrica finita):
\[
\sum_{r=0}^{N}q^r=\frac{1-q^{N+1}}{1-q}\qquad(q\neq1).
\]
Con $N=n-1-m$:
\[
\sum_{j=m}^{n-1}q^j
=q^m\frac{1-q^{n-m}}{1-q}
=\frac{q^m-q^n}{1-q}.
\]
Entonces
\[
|x_n-x_m|\le\frac{q^m-q^n}{1-q}\,|x_1-x_0|
           \le\frac{q^m}{1-q}\,|x_1-x_0|.
\]
Como $q<1$, el lado derecho tiende a $0$ cuando $m\to\infty$, así $\{x_k\}$ es de Cauchy.
...
\begin{proof}
Para todo $n>k$ se tiene la identidad telescópica finita
\[
x_n-x_k=\sum_{j=k}^{n-1}(x_{j+1}-x_j).
\]
Como $x_n\to x^*$, al tomar límite en $n$:
\[
x^*-x_k=\sum_{j=k}^{\infty}(x_{j+1}-x_j),
\]
y por tanto
\[
|x_k-x^*|\le\sum_{j=k}^{\infty}|x_{j+1}-x_j|
=\sum_{j=k}^{\infty}|g(x_j)-g(x_{j-1})|
\le\sum_{j=k}^{\infty}q|x_j-x_{j-1}|
...
Si $x_0\in[x^*,b]$ y $f(x_0)\ge 0$ (condición de Fourier), entonces:
\begin{enumerate}[(i)]
...
Sea
\[
g(x)=x-\frac{f(x)}{f'(x)},\qquad x_{k+1}=g(x_k).
\]
Como $f$ es continua, $x^*$ es la única raíz y $f(x_0)\ge0$ con $x_0\ge x^*$,
se tiene $f(x)\ge0$ para todo $x\in[x^*,x_0]$.

\textit{Paso 1 (función creciente).}
Si $x\in(x^*,x_0]$, entonces $f(x)>0$ (unicidad de raíz).
Por el Teorema del Valor Medio entre $x^*$ y $x$, existe $\xi\in(x^*,x)$ tal que
\[
f'(\xi)=\frac{f(x)-f(x^*)}{x-x^*}=\frac{f(x)}{x-x^*}>0.
\]
Como $f''>0$, $f'$ es creciente, luego $f'(x)\ge f'(\xi)>0$.

\textit{Paso 2 (cota inferior).}
Tomemos $x_k\in[x^*,x_0]$. La convexidad da
\[
f(x^*)\ge f(x_k)+f'(x_k)(x^*-x_k),
\]
y como $f(x^*)=0$:
\[
0\ge f(x_k)+f'(x_k)(x^*-x_k).
\]
Además, por el Paso 1, $f'(x_k)>0$, y podemos dividir:
\[
x_{k+1}=x_k-\frac{f(x_k)}{f'(x_k)}\ge x^*.
\]

\textit{Paso 3 (monotonía).}
Como $f$ es creciente en $(x^*,x_0]$ y $x_k\ge x^*$, se tiene $f(x_k)\ge0$.
Entonces
\[
x_{k+1}=x_k-\frac{f(x_k)}{f'(x_k)}\le x_k.
\]
Por lo tanto, $x^*\le x_{k+1}\le x_k$.
Repitiendo este paso desde $x_0$, la sucesión es decreciente y acotada inferiormente por $x^*$,
luego converge a algún $\ell\ge x^*$.

Además, $g$ es continua en un entorno de $\ell$ (si $\ell>x^*$, por el mismo argumento
del Paso 1 se tiene $f'(\ell)>0$), y por tanto
\[
\ell=\lim_{k\to\infty}x_{k+1}=\lim_{k\to\infty}g(x_k)=g(\ell).
\]
Así, $f(\ell)=0$, contradicción con la unicidad de la raíz y $\ell>x^*$.
Por lo tanto, $\ell=x^*$.
La convergencia cuadrática se sigue del Teorema~\ref{thm:newton_1d_conv}.
\end{proof}

\begin{corollary}[Caso cóncavo (simétrico)]\label{cor:newton_concavo}
Sea $f\in C^2([a,b])$ con $f''(x)<0$ para todo $x\in[a,b]$,
y sea $x^*\in(a,b)$ la única raíz de $f$ en ese intervalo.
Si $x_0\in[a,x^*]$ y $f(x_0)\le 0$ (condición de Fourier), entonces:
\begin{enumerate}[(i)]
\item Los iterados satisfacen $x_k\le x_{k+1}\le x^*$ para todo $k\ge 0$.
\item La sucesión $\{x_k\}$ converge monótonamente a $x^*$.
\item La convergencia es cuadrática.
\end{enumerate}
En particular, este es el caso de la condición de Fourier
$f(x_0)<0$ y $f''(x_0)<0$.
\end{corollary}

\begin{proof}
Aplicamos el Teorema~\ref{thm:newton_convex} a $\tilde f=-f$, que es estrictamente convexa.
La iteración de Newton no cambia porque
\[
x-\frac{\tilde f(x)}{\tilde f'(x)}=x-\frac{-f(x)}{-f'(x)}=x-\frac{f(x)}{f'(x)}.
\]
Además, $\tilde f(x_0)\ge 0$ y $\tilde f''(x_0)>0$, de modo que se obtiene la conclusión
monótona y la convergencia cuadrática para la misma sucesión $\{x_k\}$.
\end{proof}
