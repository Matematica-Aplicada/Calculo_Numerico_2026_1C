...
\]
Además, si $f\in C^4([a,b])$, demuestre que el error puede escribirse como
\[
\int_a^b f(x)\,dx-\frac{b-a}{6}\left(f(a)+4f\left(\frac{a+b}{2}\right)+f(b)\right)=-\frac{(b-a)^5}{2880}f^{(4)}(\xi), \qquad \text{para algún } \xi\in(a,b).
\]
\end{enumerate}
\end{exercise}
...
\]
\item Justifique los órdenes $O(h)$ y $O(h^2)$ utilizando desarrollos de Taylor de $f$ alrededor de $x_0$.
\end{enumerate}
\end{exercise} 
...
\begin{exercise}[Óptica de Fourier: filtrado espacial]
En un sistema óptico \emph{4f}, una lente transforma el campo de entrada $u\in\mathbb{C}^N$ en su DFT $\hat{u}$ en el plano focal (llamado \emph{plano de Fourier}).  Allí se coloca una máscara de transmitancia $H\in\mathbb{C}^N$ que multiplica componente a componente: $\hat{v}_k = H_k\,\hat{u}_k$.  Una segunda lente aplica la DFT inversa para obtener el campo de salida $v$.
\begin{enumerate}[(a)]
\item Demuestre que $v = h * u$ (convolución circular), donde $h$ es la DFT inversa de $H$.  La función $h$ se llama \emph{respuesta al impulso} del sistema.
\item Filtro pasa-bajos. Sea $N$ par y defina $H_k = 1$ si la frecuencia reindexada satisface $|k|\le k_c$ y $H_k = 0$ en caso contrario (usando la convención $k\in\{-N/2,\ldots,N/2-1\}$).  Explique por qué este filtro suaviza la señal y elimina detalles finos.
\item \textbf{Conservación de energía.}  Usando Parseval, demuestre que si $|H_k|\le 1$ para todo $k$, entonces $\|v\|^2 \le \|u\|^2$.  ¿Qué ocurre si $|H_k|=1$ para todo $k$ (máscara de fase pura)?
\end{enumerate}
\end{exercise}
