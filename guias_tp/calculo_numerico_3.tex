...
\begin{enumerate}[(a)]
\item Verifique que $g \notin C^0([0,1])$ y, en particular, que $g$ no es acotada cerca de $x = 0$. ¿Qué orden de convergencia se esperaría al aplicar la regla del trapecio compuesta directamente a $\int_0^1 g(x)\,dx$?
...
\item ¿Qué orden de convergencia se obtiene al aplicar la regla del trapecio compuesta a $\int_0^1 \varphi(x)\,dx$? Compare con el resultado del inciso (a). ¿Qué se ganó al extraer la singularidad?
\end{enumerate}
\end{exercise}
...
\]
Demuestre formalmente que $Q_n$ es exacta para todo polinomio de grado menor o igual que $n$, es decir:
\[
