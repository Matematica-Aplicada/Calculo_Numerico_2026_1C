...
Sea además $f \in C^2([x_0,x_n])$ tal que $f(x_i)=y_i$ para $i=0,\ldots,n$.
\begin{enumerate}[(a)]
...
\item Defina el error de cuadratura como $E(f)=I(f)-Q(f)$. Obtenga una expresión para $E(f)$ en términos del error de interpolación de grado 2. \textbf{Sugerencia}: Escriba $f(x)=p_2(x)+e(x)$, donde $p_2$ es el polinomio interpolante de $f$ en los nodos $\{0,1/2,1\}$ y $e(x)=f(x)-p_2(x)$. Como $Q$ es exacta sobre polinomios de grado $\le 2$, deduzca que
\[
