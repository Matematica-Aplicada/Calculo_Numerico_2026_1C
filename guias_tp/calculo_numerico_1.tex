...
Demuestre que \textbf{no existe tal algoritmo}:
\begin{enumerate}[(a)]
...
\begin{enumerate}
\item Demuestre que $\text{fl}(\text{fl}(x) + \text{fl}(y)) \approx (x+y)(1 + \delta)$ con $|\delta| \leq \varepsilon_{2\text{mach}}$ si no hay cancelación.
\item Demuestre que $\text{fl}(\text{fl}(x) \times \text{fl}(y)) \approx xy(1 + \delta)$ con $|\delta| \leq 2\varepsilon_{3\text{mach}}$.
\item ¿Por qué el error relativo puede ser arbitrariamente grande en la suma si $x + y \approx 0$?
\end{enumerate}
\end{exercise}

% Como está planteado el ejercicio no hay pérdida de dígitos significativos. Los ítem a) y b) dan los mismos resultados.
