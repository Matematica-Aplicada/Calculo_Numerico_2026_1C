...
\section*{I. Series de Fourier}
\subsection*{I.1 Aproximación de funciones}
...
\bej
(Cota mejorada del error de truncamiento de Fourier)
Sea $f \in C^n_{per}([0,2\pi])$ con $n \geq 2$. Sabemos que los coeficientes de Fourier satisfacen $\hat{f}_k = \widehat{f^{(n)}}_k / (ik)^n$, de modo que $|\hat{f}_k| = |\widehat{f^{(n)}}_k| / |k|^n$.
\begin{enumerate}[(a)]
    \item Muestre que, usando la desigualdad de Cauchy-Schwarz:
    \[
    ...
    \]
    \item Acote el primer factor usando la desigualdad de Bessel aplicada a $f^{(n)}$ con la base ortonormal $\{e^{ikx}/\sqrt{2\pi}\}$, para obtener:
    \[
    \left(\sum_{|k| > N} |\widehat{f^{(n)}}_k|^2\right)^{1/2} \leq \frac{\|f^{(n)}\|_{L^2}}{\sqrt{2\pi}}.
    \]
    \item Acote el segundo factor comparando la serie con una integral:
    \[
    \sum_{|k| > N} \frac{1}{|k|^{2n}} \leq \frac{2}{(2n-1)N^{2n-1}}.
    \]
    \item Combine los resultados anteriores para concluir que:
    \[
    \|f - S_N f\|_\infty \leq \sum_{|k| > N} |\hat{f}_k| \leq \frac{\|f^{(n)}\|_{L^2}}{\sqrt{\pi(2n-1)}} \cdot \frac{1}{N^{n - 1/2}}.
    \]
    \item Compare esta tasa $O(N^{1/2-n})$ con la $O(N^{1-n})$ que se obtiene por comparación directa con una integral (sin usar Cauchy-Schwarz ni Bessel). ¿De dónde proviene la ganancia de $N^{-1/2}$?
\end{enumerate}
\fej
...
\bej
(De Fourier a la DFT)
Considere los coeficientes de Fourier en forma compleja $c_k = \frac{1}{2\pi} \int_0^{2\pi} f(x) e^{-ikx} \, dx$. Muestre que al aproximar la integral utilizando la regla de los trapecios compuesta con $N$ puntos $T_N$ se obtiene Transformada Discreta de Fourier (DFT) de la secuencia de valores muestreados $f(x_j)$, salvo posibles factores de normalización.
\fej

\subsection*{I.2 Métodos espectrales para ODEs periódicas}

\bej
(Modelo modal, error de truncamiento y estabilidad)
Considere una ecuación lineal periódica cuya solución en Fourier satisface
\[
\hat u_k = T_k\,\hat f_k,\qquad k\in\mathbb Z,
\]
donde $T_k$ es el \textbf{factor de transferencia} (o multiplicador espectral).
Defina el truncado
\[
u_N(t)=\sum_{|k|\le N}\hat u_k e^{ikt}.
\]
\begin{enumerate}[(a)]
    \item Usando Parseval, pruebe que
    \[
    \|u-u_N\|_{L^2(0,2\pi)}^2=2\pi\sum_{|k|>N}|T_k|^2|\hat f_k|^2.
    \]
    \item Deduzca la cota
    \[
    \|u-u_N\|_{L^2}\le \sqrt{2\pi}\,\sup_{|k|>N}|T_k|\left(\sum_{|k|>N}|\hat f_k|^2\right)^{1/2}.
    \]
    \item Defina la constante de estabilidad truncada
    \[
    C_{\mathrm{est}}(N):=\sup_{|k|\le N}|T_k|,
    \]
...
\bej
(ODE forzada periódica: orden de convergencia y estabilidad)
Considere
\[
y''(t)+2\gamma y'(t)+\omega_0^2y(t)=f(t),\qquad t\in[0,2\pi],
\]
con periodicidad y $f(t)=\sum_{k\in\mathbb Z}\hat f_k e^{ikt}$.
\begin{enumerate}[(a)]
    \item Muestre que
    \[
    ...
    \item Estudie el comportamiento de $|H_k|$ para $|k|\to\infty$ y concluya cómo cambia el \textbf{orden de convergencia} de $y_N$ respecto del orden de convergencia de la serie de Fourier de $f$.
...
\bej
(Ejemplo concreto de ganancia de orden)
Considere
\[
...
\begin{enumerate}[(a)]
    \item Calcule (o recuerde) el orden de decaimiento de $|\hat f_k|$.
    \item Muestre que
    \[
    \hat y_k=\frac{\hat f_k}{1+k^2}
    \]
    y deduzca el orden de decaimiento de $|\hat y_k|$.
    \item Determine el \textbf{orden de convergencia} de $\|y-y_N\|_{L^2}$ y compárelo con el orden de $\|f-f_N\|_{L^2}$.
\end{enumerate}
\fej

\subsection*{I.3 Ecuaciones integrales e integro-diferenciales (convolución)}

\bej
(Convolución de primer tipo: pérdida de $r$ órdenes)
Sea
\[
Ku=f,
\qquad
...
\[
|\hat k_n|\asymp |n|^{-r},\qquad r>0,
\]
para $|n|$ grande.
\begin{enumerate}[(a)]
    \item Muestre que $\hat u_n=\hat f_n/\hat k_n$ y, si $|\hat f_n|\asymp |n|^{-p}$, deduzca
    \[
    |\hat u_n|\asymp |n|^{-(p-r)}.
    \]
    \item Compare el orden de convergencia de $u_N$ con el de $f_N$ en norma $L^2$ (suponga los exponentes necesarios para que ambas series estén en $L^2$).
    \item Interprete el resultado: la inversión de primer tipo \textbf{pierde $r$ órdenes} respecto de aproximar el lado derecho.
\end{enumerate}
\fej

\bej
(Convolución de segundo tipo con las mismas hipótesis)
Considere ahora
\[
u+Ku=f,
\qquad
(Ku)(x)=\frac{1}{2\pi}\int_0^{2\pi}k(x-t)u(t)\,dt,
\]
con el mismo supuesto $|\hat k_n|\asymp |n|^{-r}$.
\begin{enumerate}[(a)]
    \item Muestre que
    \[
    \hat u_n=T_n\hat f_n,
    ...
    \]
    \item Pruebe una cota de truncamiento
    \[
    \|u-u_N\|_{L^2}\le \sqrt{2\pi}\,\sup_{|n|>N}|T_n|\left(\sum_{|n|>N}|\hat f_n|^2\right)^{1/2}.
    \]
    \item Explique por qué, para $|n|$ grande, $T_n\to 1$ y por lo tanto el método conserva el orden de convergencia de la aproximación de $f$ (salvo modos casi resonantes de baja frecuencia, donde la estabilidad puede degradarse).
\end{enumerate}
\fej

\bej
(Integro-diferencial I: una derivada fuera de la integral)
Considere
\[
u(t)+\frac{d}{dt}\left[\frac{1}{2\pi}\int_0^{2\pi}k(t-s)u(s)\,ds\right]=f(t),
\]
y suponga nuevamente $|\hat k_n|\asymp |n|^{-r}$.
\begin{enumerate}[(a)]
    \item Muestre que
    \[
    \hat u_n=T_n\hat f_n,
    ...
    \]
    \item Escriba la cota de truncamiento para $\|u-u_N\|_{L^2}$ en términos de $T_n$.
    \item Según el valor de $r$, analice el comportamiento asintótico de $|T_n|$ y concluya si se espera ganancia, pérdida o conservación de orden respecto de la aproximación de $f$.
\end{enumerate}
\fej

\bej
(Integro-diferencial II: operador hypersingular de primer tipo)
Considere la ecuación de primer tipo
\[
\frac{d}{dt}\left[\frac{1}{2\pi}\int_0^{2\pi}k(t-s)u'(s)\,ds\right]=f(t),
\]
con periodicidad en $[0,2\pi]$ y suponga $|\hat k_n|\asymp |n|^{-r}$.
\begin{enumerate}[(a)]
    \item Muestre que
    \[
    ...
    \item Defina
    \[
    C_{\mathrm{est}}(N)=\sup_{1\le |n|\le N}|T_n|,
    \]
    y discuta cuándo esta constante es buena y cuándo puede ser grande (modos con $\hat k_n$ pequeños).
    \item Escriba una cota para $\|u-u_N\|_{L^2}$ y, usando $|\hat k_n|\asymp |n|^{-r}$, compare el orden esperado de convergencia de $u_N$ con el de la aproximación de $f$.
    ¿Para qué valores de $r$ hay ganancia de orden, conservación o pérdida?
\end{enumerate}
\fej

\bej
(Núcleo logarítmico periódico: descomposición en modos de Fourier)
Considere el núcleo periódico
\[
K(x,y):=-\log\!\left(2\sin\frac{|x-y|}{2}\right),
\qquad x,y\in[0,2\pi].
\]
Use como \textbf{hecho dado} la identidad (para $t\in(0,2\pi)$):
\[
...
\begin{enumerate}[(a)]
    ...
    \]
    \item Reescriba la expansión en forma separada:
    \[
    K(x,y)=\sum_{n=1}^{\infty}\frac{1}{n}\big(\cos(nx)\cos(ny)+\sin(nx)\sin(ny)\big).
    \]
    Identifique los valores espectrales (coeficientes) asociados a cada modo.
    \item Para el operador integral
    \[
    (Ku)(x)=\frac{1}{2\pi}\int_0^{2\pi}K(x,y)u(y)\,dy,
    \]
    escriba la relación modal entre $\hat u_n$ y $\widehat{Ku}_n$, y determine el orden de decaimiento del multiplicador.
    \item Interprete la singularidad logarítmica de $K$ en $x=y$ y discuta por qué sigue siendo integrable.
\end{enumerate}
\fej

\section*{II. Tchebyshev}

\subsection*{II.1 Aproximación de funciones}
